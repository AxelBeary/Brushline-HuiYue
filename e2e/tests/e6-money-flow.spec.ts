...
test('E6 金钱链路完整流程', async ({ page, artistPage }) => {
  ...
  const order = items.find((o: { order_no: string }) => o.order_no === orderNo)
...
test('E6b SPEC-PRICE-2 风格模式算价+下单+收款', async ({ page, artistPage }) => {
  ...
  const mkTpl = async (data: Record<string, string | number>) => {
  ...
  const saOf = (tplId: number) => bound.find((a: { addon_template_id: number }) => a.addon_template_id === tplId)
  ...
  const saUsage2 = bound2.find((s: { id: number }) => s.id === style.id)!.addons.find((a: { addon_template_id: number }) => a.addon_template_id === tplUsage2.id)
  ...
  const listRes = await artistPage.request.get(base + '/api/artist/orders?pageSize=200')
  const order = (await listRes.json()).items.find((o: { order_no: string }) => o.order_no === orderNo)
  expect(order).toBeTruthy()
