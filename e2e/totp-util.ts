...
export function totpForCounter(secretBase32: string, counter: number): string {
  const key = base32Decode(secretBase32)
  const msg = Buffer.alloc(8)
  msg.writeUInt32BE(Math.floor(counter / 0x100000000), 0)
  msg.writeUInt32BE(counter >>> 0, 4)
  // 原 JS 对 null 密钥即在 crypto.createHmac 处运行时抛错，此处 as 断言保持同一行为
  const hash = crypto.createHmac('sha1', key as Buffer).update(msg).digest()
 ...
 */
export function nextStepTotp(secretBase32: string): string {
...
const usedLoginCounters = new Map<string, Set<number>>()

/**
 * 生成一次“刷新登录”可用的 TOTP 候选（±1 窗口，跳过本进程已消费的时间步）。
 * 返回 [{ counter, code }]，调用方逐个尝试，命中后须调用 noteTotpLogin 记录。
 */
export function nextLoginTotp(qqNumber: string): Array<{ counter: number; code: string }> {
  const now = Math.floor(Date.now() / 1000 / 30)
  const used = usedLoginCounters.get(qqNumber) ?? new Set<number>()
  const candidates: Array<{ counter: number; code: string }> = []
...
export function noteTotpLogin(qqNumber: string, counter: number): void {
  const used = usedLoginCounters.get(qqNumber) ?? new Set<number>()
  used.add(counter)
