...
function createLegacyReferenceFile(filePath) {
...
describe('审计批 F-10 参考图归属凭据', () => {
  ...
  function orderPayload(filePath) {
    return {
  ...
  it('TC-F10-03: 有效 token 上传成功并登记 (anon_id, file_path, order_id=NULL)', async () => {
    ...
    expect(res.statusCode).toBe(200)
    const filePath = res.json().filePath
    expect(filePath).toMatch(/^references\//)

    const row = db.prepare('SELECT anon_id, file_path, order_id FROM reference_uploads WHERE file_path = ?').get(filePath)
    expect(row).toBeTruthy()
    expect(row.order_id).toBeNull()
    const anonId = db.prepare('SELECT id FROM anon_tokens WHERE token = ?').get(token).id
  ...
  it('TC-F10-04: A token 上传的文件，B token 下单挂不上（ILLEGAL_PATH，无半态）', async () => {
    ...
    const filePath = upload.json().filePath
    ...
    const row = db.prepare('SELECT order_id FROM reference_uploads WHERE file_path = ?').get(filePath)
    expect(row.order_id).toBeNull()
  })
  ...
  it('TC-F10-05: 同 token 正常下单并绑定，随后不可二次使用', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) AS c FROM order_references WHERE order_id = ? AND file_path = ?').get(order.id, filePath).c).toBe(1)
  ...
  it('TC-F10-08: 存量未登记路径（v55 前文件）放行，不要求登记记录', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) AS c FROM reference_uploads WHERE file_path = ?').get(legacyPath).c).toBe(0)
  })
