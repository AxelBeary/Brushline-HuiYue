...
type StyleSize = ReturnType<typeof styleService.createStyleSize>
type Artwork = NonNullable<Awaited<ReturnType<typeof artistService.createArtwork>>>

function setMultiStyleEnabled(artistId: number, enabled: number) {
...
describe('迁移 v37 结构验证', () => {
  ...
  it('TC-V37-01: style_sizes 新字段存在（image/image_artwork_id/description/work_days）', () => {
    const cols = (db.prepare('PRAGMA table_info(style_sizes)').all() as Array<{ name: string }>).map(c => c.name)
  ...
  it('TC-V37-03: artworks.description 存在', () => {
    const cols = (db.prepare('PRAGMA table_info(artworks)').all() as Array<{ name: string }>).map(c => c.name)
    expect(cols).toContain('description')
  })
  ...
  it('TC-V37-04: artwork_size_tags 表存在 + 双向 CASCADE', async () => {
    ...
    const remaining = (db.prepare('SELECT COUNT(*) AS c FROM artwork_size_tags').get() as { c: number }).c
  ...
  it('TC-V37-05: 删作品 → 引用它的尺寸 image_artwork_id 自动置空', async () => {
    ...
    artistService.deleteArtwork(artwork.id)
    const row = db.prepare('SELECT image_artwork_id FROM style_sizes WHERE id = ?').get(size.id) as { image_artwork_id: number | null }
