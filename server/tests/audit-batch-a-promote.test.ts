...
describe('audit-a R-1 自动递补容错', () => {
  ...
  function makePriceOrder(orderNo: string, zone: string, status: string) {
  ...
  function makeDirtyBuffer(orderNo: string) {
  ...
  it('TC-R1-01: 交付正式单触发递补时，脏缓冲单不炸主流程且留在 buffer', () => {
    ...
    const delivered = db.prepare('SELECT status FROM orders WHERE id = ?').get(formal.id) as { status: string }
    expect(delivered.status).toBe('delivered')
    const bufferZone = db.prepare('SELECT queue_zone FROM orders WHERE id = ?').get(dirty.id) as { queue_zone: string }
  ...
  it('TC-R1-02: 健康缓冲单递补回归不破（auto_promote 正常生效）', () => {
    ...
    const promoted = db.prepare('SELECT queue_zone FROM orders WHERE id = ?').get(buffer.id) as { queue_zone: string }
...
describe('audit-a R-2 取消已收款订单', () => {
  ...
  it('TC-R2-01: 已收款订单直接取消 → CANCEL_WITH_PAYMENT 409 + detail.paidCents', () => {
    ...
    expect(e.code).toBe('CANCEL_WITH_PAYMENT')
    expect(e.statusCode).toBe(409)
    expect(e.detail).toEqual({ paidCents: 5000 })
    // 状态未被写入
    expect((db.prepare('SELECT status FROM orders WHERE id = ?').get(order.id) as { status: string }).status).toBe('wip')
  })
