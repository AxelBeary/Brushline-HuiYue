...
function fourNodes(amounts = [0, 0, 0, 0], paid = [0, 0, 0, 0]) {
...
describe('案例 10：单节点 30%，ratioTotal 按 30% 算', () => {
  it('TC-PE-C10-01: allocateInitial 只摊总价的 30%', () => {
    const nodes = [{ sortOrder: 0, basisPoints: 3000, amountCents: 0 }]
...
describe('assertConservation 三条断言的破坏检测（R11）', () => {
  ...
  it('TC-PE-CONS-02: 破坏 A2（总价 − 已收 ≠ Σ待收 + 额外应收 − 额外应退）', () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(AppError)
      expect(err.code).toBe(E.PRICING_CONSERVATION)
      expect(err.detail.assertion).toBe('A2')
    }
  })
  ...
  it('TC-PE-CONS-03: 破坏 A3（节点价 ≠ 基础分摊额 + Σ历次分摊增量）', () => {
    ...
    try {
      ...
      expect(err.code).toBe(E.PRICING_CONSERVATION)
      expect(err.detail.assertion).toBe('A3')
      expect(err.detail.index).toBe(2)
    }
  })
...
describe('迁移 v39: order_price_entries 建表', () => {
  ...
  it('TC-MIG-39b: 七种条目类型均可写入（R1）', () => {
    ...
    const rows = db.prepare('SELECT * FROM order_price_entries WHERE order_id = ? ORDER BY id').all(orderId)
  ...
  it('TC-MIG-39d: 订单删除时条目级联清除（ON DELETE CASCADE）', () => {
    ...
    const left = db.prepare('SELECT COUNT(*) AS c FROM order_price_entries WHERE order_id = ?').get(orderId)
