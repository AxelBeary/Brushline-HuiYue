...
/** service 返回行类型别名（测试辅助变量标注用） */
type ArtStyle = ReturnType<typeof styleService.createArtStyle>
type StyleSizeRow = ReturnType<typeof styleService.createStyleSize>
type AddonTemplate = ReturnType<typeof styleService.createAddonTemplate>
type StyleAddon = ReturnType<typeof styleService.getStyleAddons>[number]
...
describe('迁移 v36 老数据迁移', () => {
  ...
  it('TC-MIG-01: 空库跑 v36 → 5 表存在，无数据', () => {
    ...
    // 空库无画师 → 无迁移数据
    const styleCount = (db.prepare('SELECT COUNT(*) AS c FROM art_styles').get() as { c: number }).c
  ...
  it('TC-MIG-02: 手工构造画风/尺寸/增项数据 → getArtStyles 嵌套读取正常', () => {
    ...
    const style = db.prepare('SELECT * FROM art_styles WHERE artist_id = ?').get(artist.id) as { id: number }
    ...
    const templates = db.prepare('SELECT * FROM addon_templates WHERE artist_id = ?').all(artist.id) as Array<{ id: number }>
  ...
  it('TC-MIG-03: 迁移幂等 — 已有 art_styles 数据不重复创建', () => {
    ...
    const countBefore = (db.prepare('SELECT COUNT(*) AS c FROM art_styles WHERE artist_id = ?').get(artist.id) as { c: number }).c
    expect(countBefore).toBe(1)

    // 迁移逻辑检查：existingStyles > 0 → 跳过
    const existingStyles = (db.prepare('SELECT COUNT(*) AS c FROM art_styles').get() as { c: number }).c
...
describe('画风 CRUD (art_styles)', () => {
  ...
  it('TC-AS-05: 删除画风 — 级联删 sizes + addons + overrides', () => {
    ...
    // 验证级联删除
    expect((db.prepare('SELECT COUNT(*) AS c FROM style_sizes WHERE art_style_id = ?').get(style.id) as { c: number }).c).toBe(0)
    expect((db.prepare('SELECT COUNT(*) AS c FROM style_addons WHERE art_style_id = ?').get(style.id) as { c: number }).c).toBe(0)
    expect((db.prepare('SELECT COUNT(*) AS c FROM size_addon_overrides WHERE style_size_id = ?').get(size.id) as { c: number }).c).toBe(0)
  })
...
describe('尺寸 CRUD (style_sizes)', () => {
  let artist: ArtistRow, style: ArtStyle
  ...
  it('TC-SS-03b: 后置校验失败时 name 变更一并回滚（L-10）', () => {
    ...
    const row = db.prepare('SELECT name, display_status FROM style_sizes WHERE id = ?').get(size.id) as { name: string; display_status: string }
...
describe('画风增项批量设置 (style_addons)', () => {
  let artist: ArtistRow, style: ArtStyle, tpl1: AddonTemplate, tpl2: AddonTemplate
...
describe('尺寸覆盖 (size_addon_overrides)', () => {
  let artist: ArtistRow, style: ArtStyle, size: StyleSizeRow, tpl: AddonTemplate, styleAddon: StyleAddon
...
describe('多画风路由层集成测试', () => {
  ...
  it('TC-RT-03: 画风 CRUD + 尺寸 + 增项完整链路', async () => {
    ...
    expect(style.addons.filter((a: { template_category: string }) => a.template_category === 'add')).toHaveLength(1)
    ...
    // 批量设置增项（改价）
    const tplId = style.addons.find((a: { template_category: string }) => a.template_category === 'add').addon_template_id
    ...
    const targetSa = styleAddons.find((a: { addon_template_id: number }) => a.addon_template_id === tplId)
    ...
    expect(styles[0].addons.filter((a: { template_category: string }) => a.template_category === 'add')).toHaveLength(1)
  })
...
describe('SPEC-PRICE-2 画风增项解绑与覆盖读取', () => {
  ...
  function setupScene(qq: string, subdomain: string) {
...
describe('audit-batch-b P3-29 金额写入两位小数约束', () => {
  ...
  it('TC-P3-29-03: 增项 price_override（画风/尺寸覆盖）——600.005 → 400；600.01 → 200', async () => {
    ...
    const sa = okAddons.json().find((a: { addon_template_id: number }) => a.addon_template_id === tpl.id)
