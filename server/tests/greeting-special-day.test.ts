...
/** 直接插特别日（返回 id） */
function seedDay({ name = '测试日', dateKey = '08-14', artistId = null, enabled = 1 }: { name?: string; dateKey?: string; artistId?: number | null; enabled?: number }) {
...
/** 直接插文案（可挂特别日） */
function seedTemplate({ text, slot = 'any', artistId = null, specialDayId = null, enabled = 1 }: { text: string; slot?: string; artistId?: number | null; specialDayId?: number | null; enabled?: number }) {
  const result = db.prepare(
...
describe('E5 问候池扩展：深夜池 + 特别日池', () => {
  ...
  describe('getTodayDateKey / isValidDateKey', () => {
    ...
    it('TC-E5-03: 非法日期被拒（月/日越界、格式残缺、非字符串）', () => {
      for (const key of ['13-01', '00-10', '08-00', '08-32', '8-14', '08/14', '08-1', '', null, 814] as unknown[]) {
  ...
  describe('特别日 CRUD', () => {
    ...
    it('TC-E5-22: deleteSpecialDay 级联删除关联文案', () => {
      ...
      // 未挂该日的文案不受影响
      expect((db.prepare('SELECT COUNT(*) AS c FROM greeting_templates').get() as { c: number }).c).toBe(1)
    })
