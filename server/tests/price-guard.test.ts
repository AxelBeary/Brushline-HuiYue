...
describe('v0.37 价格守卫 (Price Guard)', () => {
  ...
  function makeArtist(qq = '88101', sub = 'price-guard'): ArtistRow {
  ...
  function putPrice(app: FastifyInstance, orderId: number, headers: Record<string, string>, cents = 60000) {
    return app.inject({
      method: 'PUT',
  ...
  it('TC-PG-01: delivered 订单改价 → 400 ORDER_FINAL_STATE', async () => {
    ...
    // 价格未被改动
    const row = db.prepare('SELECT final_price_cents FROM orders WHERE id = ?').get(order.id) as { final_price_cents: number | null }
  ...
  async function seedOrderWithItem(artist: ArtistRow, status: string): Promise<{ orderId: number; itemId: number }> {
    ...
    const itemId = addRes.json().extraItems[0].id as number
    // 再改状态
  ...
  it('TC-PG-05: delivered 订单删除附加项 → 400 ORDER_FINAL_STATE', async () => {
    ...
    // 附加项未被删除
    const row = db.prepare('SELECT COUNT(*) as c FROM order_extra_items WHERE id = ?').get(itemId) as { c: number }
    expect(row.c).toBe(1)
  })
