...
describe('TOTP 重放防护 (P1-1)', () => {
  ...
  it('TC-REPLAY-01: 同一时间窗口内同一验证码第二次使用被拒', () => {
    ...
    const second = verifyTotpLogin('20001', code)
  ...
  it('TC-REPLAY-03: 重放被拒不累计防爆破计数、不锁定', () => {
    ...
    // 失败计数仍为 0、未锁定
    const row = db.prepare('SELECT totp_failed_attempts, totp_locked_until FROM artists WHERE id = ?').get(artist.id)
