...
describe('SPEC-003 附加工作项 (Extra Items)', () => {
  ...
  function authH(artist: ArtistRow) {
  ...
  it('TC-EI-02: 添加 3 项 → final = base + Σ3项', async () => {
    ...
    for (const [name, cents] of [['武器', 5000], ['背景', 3000], ['特效', 2000]] as Array<[string, number]>) {
      await app.inject({
        method: 'POST',
        url: `/api/artist/orders/${order.id}/extra-items`,
        headers: authH(artist),
    ...
    const row = db.prepare('SELECT final_price_cents FROM orders WHERE id = ?').get(order.id) as { final_price_cents: number }
  ...
  it('TC-EI-09: 添加/删除后 notes 含 📎 记录', async () => {
    ...
    const addNote = notes.find((n: { content: string; created_by?: string }) => n.content.includes('📎') && n.content.includes('加一把武器'))
    ...
    const delNote = delNotes.find((n: { content: string }) => n.content.includes('📎') && n.content.includes('移除'))
