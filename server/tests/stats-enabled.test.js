...
describe('统计功能总开关 (Stats Enabled)', () => {
  ...
  function adminToken() {
    return createSession(admin.id, admin.token_version, { authLevel: 'admin_verified', adminVerifiedAt: Date.now() })
  }
  ...
  it('TC-SE-06: setStatsEnabled 服务层往返', () => {
    ...
    expect(db.prepare("SELECT value FROM platform_config WHERE key = 'stats_enabled'").get().value).toBe('1')
    expect(getStatsEnabled()).toBe(true)
