...
describe('迁移 v67 旧数据搬家', () => {
  ...
  it('TC-MV67-03: 六档按规则搬家（night/latenight 合并 midnight），行数不丢', () => {
    ...
    const rows = mem.prepare('SELECT text, time_slot, artist_id FROM greeting_templates ORDER BY id').all()
    expect(rows).toHaveLength(8)
    const slotOf = Object.fromEntries(rows.map(r => [r.text, r.time_slot]))
  ...
  it('TC-MV67-04: 归属（专属/全局）、启停、特别日关联、索引不变', () => {
    ...
    const owned = mem.prepare("SELECT artist_id FROM greeting_templates WHERE text = '专属夜'").get()
    expect(owned.artist_id).toBe(1)
    const special = mem.prepare("SELECT special_day_id FROM greeting_templates WHERE text = '生日文案'").get()
  ...
  it('TC-MV67-05: 幂等守卫——已是新形态时重跑跳过且清残留', () => {
    ...
    expect(mem.prepare('SELECT COUNT(*) AS n FROM greeting_templates').get().n).toBe(8)
    mem.close()
  })
})
