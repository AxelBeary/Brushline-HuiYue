...
import type { ArtistRow, OrderRow } from './setup.js'
...
function seedOrder(artistId: number, overrides: Record<string, unknown> = {}): OrderRow {
  const defaults = {
  ...
  const data = { ...defaults, ...overrides } as typeof defaults
  ...
  return db.prepare('SELECT * FROM orders WHERE id = ?').get(Number(result.lastInsertRowid)) as OrderRow
}

/** 手动把订单 version 拨回旧值（模拟双标签页拿到旧快照后另一标签已写入） */
function staleVersionOf(orderId: number): number {
  const row = db.prepare('SELECT version FROM orders WHERE id = ?').get(orderId) as { version: number }
...
describe('审计批 D-1 订单 version 乐观锁', () => {
  ...
  it('TC-D1-01: 迁移 v53 后 orders.version 列存在且新订单默认 1', () => {
    const cols = db.prepare('PRAGMA table_info(orders)').all() as Array<{ name: string; dflt_value: string | null }>
  ...
  it('TC-D1-10: compactQueue 批量重排逐条带 version，重排后 version 各 +1 且无冲突', () => {
    ...
    for (const o of [o1, o2, o3]) {
      const row = db.prepare('SELECT version, queue_position FROM orders WHERE id = ?').get(o.id) as { version: number; queue_position: number }
    ...
    ).all(artist.id) as Array<{ queue_position: number }>).map(r => r.queue_position)
  ...
  it('TC-D1-11: addPayment 走相对增量不设版本条件，但 version 照常 +1（无丢失更新问题）', () => {
    ...
    const row = db.prepare('SELECT version, paid_total_cents FROM orders WHERE id = ?').get(order.id) as { version: number; paid_total_cents: number }
