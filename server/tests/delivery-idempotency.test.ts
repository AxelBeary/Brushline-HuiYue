...
describe('交付幂等（815 审计）', () => {
  ...
  const noteCount = (orderId: number) => (db.prepare('SELECT COUNT(*) AS c FROM order_notes WHERE order_id = ?').get(orderId) as { c: number }).c
  const logCount = (orderId: number) => (db.prepare('SELECT COUNT(*) AS c FROM order_activity_logs WHERE order_id = ?').get(orderId) as { c: number }).c
  ...
  it('TC-DLV-02: 同订单同文件重复交付 → deliverables 只落一行；不同文件正常追加', () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS c FROM deliverables WHERE order_id = ?').get(order.id) as { c: number }).c).toBe(1)

    orderGalleryService.addDeliverable(order.id, 'images/test/b.png', 'b.png', 200)
    expect((db.prepare('SELECT COUNT(*) AS c FROM deliverables WHERE order_id = ?').get(order.id) as { c: number }).c).toBe(2)
  })
})
