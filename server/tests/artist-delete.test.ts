...
import type { OrderRow } from './setup.js'
...
describe('deleteArtist 软删除', () => {
  ...
  it('TC-D-03: 关联订单不受软删除影响', () => {
    ...
    // 订单仍在
    const row = db.prepare('SELECT * FROM orders WHERE id = ?').get(order.id) as OrderRow
    expect(row).not.toBeNull()
  ...
  it('TC-D-04: 关联画风不受软删除影响', () => {
    ...
    const styles = db.prepare('SELECT * FROM art_styles WHERE artist_id = ?').all(artist.id) as Array<{ name: string }>
