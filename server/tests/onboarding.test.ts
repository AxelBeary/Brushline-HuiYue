...
describe('REQ-043 I2 开张任务卡（服务层）', () => {
  ...
  it('TC-ONB-05: dismiss → 后端标记；重复 dismiss 不覆盖原时间戳', () => {
    ...
    const ts = (db.prepare('SELECT onboarding_dismissed_at FROM artists WHERE id = ?').get(artist.id) as { onboarding_dismissed_at: string | null }).onboarding_dismissed_at
    expect(ts).not.toBeNull()

    dashboard.dismissOnboarding(artist.id)
    const again = (db.prepare('SELECT onboarding_dismissed_at FROM artists WHERE id = ?').get(artist.id) as { onboarding_dismissed_at: string | null }).onboarding_dismissed_at
...
describe('REQ-043 I2 开张任务卡（路由层）', () => {
  ...
  it('TC-ONB-08: POST dismiss 写标记；带附加字段被 schema 拒绝（400）', async () => {
    ...
    const row = db.prepare('SELECT onboarding_dismissed_at FROM artists WHERE id = ?').get(artist.id) as { onboarding_dismissed_at: string | null }
