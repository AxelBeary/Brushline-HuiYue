...
/** artist_workflow_stages 行（测试仅消费 id） */
interface StageRow {
  id: number
}

/** P2-F8: 在临时上传目录真实落盘交付文件，返回相对路径（交付前校验文件存在） */
function ensureDeliverable(artistId: number, relName: string): string {
 ...
 */
function seedTwoStageArtist(qq: string = '88011', sub: string = 'b1-two'): { artist: ArtistRow; stageIds: number[] } {
...
describe('批1 状态机修复（R1/R2/R3）', () => {
  ...
  function advanceLegally(orderId: number, stages: StageRow[], toIdx: number): void {
  ...
  it('TC-B1-04: done 回退被拒（REQ-025 R13：done 后禁止工作流回退）', () => {
    ...
    ).all(artist.id) as StageRow[]

    // 逐级合法推进到末节点 → done
    advanceLegally(order.id, stages, stages.length - 1)
    const done = orderService.getOrder(order.id) as OrderDetail
    ...
    const after = orderService.getOrder(order.id) as OrderDetail
    expect(after.status).toBe('done')
    expect(after.current_stage_id).toBe(stages[stages.length - 1].id)
    const notes = db.prepare("SELECT COUNT(*) AS c FROM order_notes WHERE order_id = ? AND content LIKE '%↩%'").get(order.id) as { c: number }
  ...
  it('TC-B1-05: confirmed 回退被拒（confirmed→revision 非法，状态机拦截在写库前）', () => {
    ...
    expect((orderService.getOrder(order.id) as OrderDetail).status).toBe('confirmed')
  ...
  it('TC-B1-06: wip 回退到 revision 合法', () => {
    ...
    expect((orderService.getOrder(order.id) as OrderDetail).status).toBe('wip')
  ...
  it('TC-B1-07: 改价1000→加项1500→改回1000→删项 抛 INVALID_PRICE 且订单数据无损', () => {
    ...
    expect((orderService.getOrder(order.id) as OrderDetail).final_price_cents).toBe(2500)
    orderService.updateFinalPrice(order.id, 1000)
    expect((orderService.getOrder(order.id) as OrderDetail).final_price_cents).toBe(1000)

    const itemId = (db.prepare('SELECT id FROM order_extra_items WHERE order_id = ?').get(order.id) as { id: number }).id
    const notesBefore = (db.prepare('SELECT COUNT(*) AS c FROM order_notes WHERE order_id = ?').get(order.id) as { c: number }).c
    const entriesBefore = (db.prepare('SELECT COUNT(*) AS c FROM order_price_entries WHERE order_id = ?').get(order.id) as { c: number }).c
    ...
    const after = orderService.getOrder(order.id) as OrderDetail & { extraItems: { price_cents: number }[] }
    ...
    expect((db.prepare('SELECT COUNT(*) AS c FROM order_notes WHERE order_id = ?').get(order.id) as { c: number }).c).toBe(notesBefore)
    expect((db.prepare('SELECT COUNT(*) AS c FROM order_price_entries WHERE order_id = ?').get(order.id) as { c: number }).c).toBe(entriesBefore)
  })
})
