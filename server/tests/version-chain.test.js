...
describe('F5 订单 version 版本链补全', () => {
  ...
  const staleVersionOf = (orderId) =>
    db.prepare('SELECT version FROM orders WHERE id = ?').get(orderId).version - 1

  function seedReference(orderId, path) {
  ...
  it('TC-F5-02: reorderQueue 参与行 version 均 +1，位置按传入顺序', () => {
    ...
    for (const o of [o1, o2, o3]) {
      expect(db.prepare('SELECT version FROM orders WHERE id = ?').get(o.id).version).toBe(o.version + 1)
    }
  ...
  it('TC-F5-03: adjustFinalPrice（addExtraItem / deleteExtraItem）写后 version 递增', () => {
    ...
    const item = db.prepare('SELECT id FROM order_extra_items WHERE order_id = ?').get(order.id)
  ...
  it('TC-F5-04: setFocusImage（small 与 off 两分支）写后 version 递增', () => {
    ...
    expect(off.focus_image_mode).toBe('off')
  })
  ...
  it('TC-F5-05: removeReference 连带清焦点图写后 version +1', () => {
    ...
    expect(updated.focus_image_mode).toBe('off')
  })
