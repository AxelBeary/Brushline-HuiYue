...
describe('REQ-039 邀请码注册（invite）', () => {
  ...
  it('TC-INV-06: 注册成功：建号 hidden + TOTP 密钥未验证 + 码一次性消费', async () => {
    ...
    const artist = db.prepare("SELECT * FROM artists WHERE qq_number = '20001'").get()
    expect(artist.status).toBe('hidden')
    expect(artist.totp_secret).toBeTruthy()
    expect(artist.totp_verified).toBe(0)

    const consumed = db.prepare('SELECT * FROM invite_codes WHERE id = ?').get(invite.id)
    expect(consumed.status).toBe('used')
    expect(consumed.used_by_artist_id).toBe(artist.id)
    expect(consumed.used_at).toBeTruthy()

    // 建号完整（须知/默认工作流已初始化）
    const rules = db.prepare('SELECT COUNT(*) AS c FROM commission_rules WHERE artist_id = ?').get(artist.id)
    expect(rules.c).toBe(1)
    const stages = db.prepare('SELECT COUNT(*) AS c FROM artist_workflow_stages WHERE artist_id = ?').get(artist.id)
  ...
  it('TC-INV-07: QQ 冲突 → QQ_TAKEN，码保持 unused（事务回滚）', async () => {
    ...
    const row = db.prepare('SELECT * FROM invite_codes WHERE id = ?').get(invite.id)
  ...
  it('TC-INV-09: 一次性消费——同码第二次注册 INVITE_INVALID', async () => {
    ...
    // 画师只有一位
    const count = db.prepare("SELECT COUNT(*) AS c FROM artists WHERE qq_number = '20004'").get()
  ...
  it('TC-INV-13: 注册后 totp-confirm 验证通过并签发会话', async () => {
    ...
    const artist = db.prepare("SELECT * FROM artists WHERE qq_number = '20007'").get()
  ...
  it('TC-INV-17: 迁移 v58 建表（invite_codes 存在 + 索引存在）', () => {
    ...
    const indexes = db.prepare("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='invite_codes'").all()
  ...
  it('TC-INV-20: registerWithInvite 事务性——子域名冲突时 TOTP 密钥不残留', async () => {
    ...
    const row = db.prepare('SELECT * FROM invite_codes WHERE id = ?').get(invite.id)
    expect(row.status).toBe('unused')
    const artist = db.prepare("SELECT * FROM artists WHERE qq_number = '40002'").get()
