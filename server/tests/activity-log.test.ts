...
import type { ActivityLog } from '../src/features/order/activity-log.service.js'
...
describe('操作日志（d3 P2 脏 JSON 容错）', () => {
  ...
  it('TC-AL-01: 单行 detail_json 非 JSON 时 detail 置 null，整页不 500', () => {
    ...
    expect((result.logs[0] as ActivityLog & { detail: unknown }).detail).toBeNull()
  ...
  it('TC-AL-02: 正常 detail_json 仍解析为对象（断言语义不降级）', () => {
    ...
    const result = getOrderLogs(order.id)
    expect((result.logs[0] as ActivityLog & { detail: unknown }).detail).toEqual({ amountCents: 1000 })
  })
})
