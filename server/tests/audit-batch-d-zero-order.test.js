...
function seedStyleSize(artistId, price) {
...
describe('审计批 D-3 零元订单显式化', () => {
  ...
  it('TC-D3-01: base_price=0 订单创建成功 + 系统备注「0 元订单：无需收款」', () => {
    ...
    const note = order.notes.find(n => n.created_by === 'system' && n.content === '0 元订单：无需收款')
  ...
  it('TC-D3-04: 非零订单不写零元备注', () => {
    ...
    expect(order.notes.some(n => n.content === '0 元订单：无需收款')).toBe(false)
  })
})
