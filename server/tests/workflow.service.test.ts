...
/** 快速给画师种入默认 7 节点 */
function seed(artistId: number) {
  wf.seedArtistStages(artistId)
  return wf.getWorkflow(artistId)
}

/** 种入一个正式区订单并生成分期快照（批4 B10 守卫的命中条件：订单行 + 分期行） */
function seedOrderWithInstallments(artistId: number, overrides: Record<string, unknown> = {}): SeededOrder {
...
function expectBlocked(fn: () => unknown, count: number) {
  let caught: unknown = null
  try { fn() } catch (err) { caught = err }
  expect(caught).toBeInstanceOf(Error)
  expect((caught as AppError).code).toBe('WORKFLOW_PAYMENT_IN_USE')
  expect((caught as AppError).detail).toEqual({ count })
}
...
describe('流程与比例服务 (Workflow Service)', () => {
  ...
  it('TC-W-11: 保存比例后总和恒 10000', () => {
    ...
    const after = wf.getWorkflow(artist.id)
    const sum = after.filter(s => s.takesPayment).reduce((a, s) => a + (s.basisPoints as number), 0)
  ...
  it('TC-W-P15b: 无活跃订单的节点正常删除', () => {
    ...
    const row = db.prepare('SELECT current_stage_id FROM orders WHERE id = ?').get(order.id) as { current_stage_id: number | null }
  ...
  it('TC-WF-G3: 活跃订单存在时 savePayment 放行并标注仅影响新订单（方案 b），新订单按新比例生成', () => {
    ...
    const oldInsts = db.prepare('SELECT basis_points FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order').all(oldOrder.id) as Array<{ basis_points: number }>
    ...
    const newInsts = db.prepare('SELECT basis_points FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order').all(newOrder.id) as Array<{ basis_points: number }>
  ...
  it('TC-WF-G7: 变更被拒后既有订单节点金额与锁定状态不变（防摊错节点）', () => {
    ...
    const before = db.prepare('SELECT amount_cents, locked, locked_reason FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order').all(order.id) as Array<{ amount_cents: number; locked: number; locked_reason: string | null }>
    const pay = stages.find(s => s.takesPayment && !s.isFinal)!
    expectBlocked(() => wf.updateStage(artist.id, pay.id, { takesPayment: false }), 1)
    expectBlocked(() => wf.reorderStages(artist.id, stages.map(s => s.id).reverse()), 1)
    // 拒绝后刷新锁定，结果与变更前一致
    orderSvc.refreshInstallmentLocks(order.id)
    const after = db.prepare('SELECT amount_cents, locked, locked_reason FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order').all(order.id) as Array<{ amount_cents: number; locked: number; locked_reason: string | null }>
