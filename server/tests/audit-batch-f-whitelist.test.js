...
describe('审计批 F-4 模板/面板白名单', () => {
  ...
  it('TC-F4-01: 合法 template_id 全部通过', () => {
    for (const tpl of ['atelier', 'classic', 'gallery', 'folio']) {
      const updated = artistService.updateArtist(artist.id, { template_id: tpl })
  ...
  it('TC-F4-03: 合法 dashboard_default_panel 通过（含 null 清除）', () => {
    for (const panel of ['queue', 'orders', 'manual', 'tiers']) {
      const updated = artistService.updateArtist(artist.id, { dashboard_default_panel: panel })
      expect(updated.dashboard_default_panel).toBe(panel)
    }
    const cleared = artistService.updateArtist(artist.id, { dashboard_default_panel: null })
