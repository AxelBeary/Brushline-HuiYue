...
describe('REQ-042 合规与内容安全', () => {
  ...
  /** 管理员绑定画师 TOTP，返回密钥 */
  function bindArtistTotp(artist) {
  ...
  function actionCount(action) {
    return db.prepare('SELECT COUNT(*) c FROM admin_actions WHERE action = ?').get(action).c
  }

  // ─── 迁移 v59 ───

  it('TC-CMP-00: 迁移 v59 就位 — reports/admin_actions 表 + artists.is_banned 默认 0', () => {
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('reports','admin_actions')").all()
    expect(tables.map(t => t.name).sort()).toEqual(['admin_actions', 'reports'])
    const cols = db.prepare('PRAGMA table_info(artists)').all()
    const banned = cols.find(c => c.name === 'is_banned')
  ...
  it('TC-CMP-04: 管理员处理举报 → resolved + admin_actions 留痕', async () => {
    ...
    const action = db.prepare("SELECT * FROM admin_actions WHERE action = 'report_resolve'").get()
  ...
  it('TC-CMP-06: 作品下架 → 删除生效 + 留痕', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(db.prepare('SELECT COUNT(*) c FROM artworks WHERE id = ?').get(artworkId).c).toBe(0)
    expect(actionCount('content_remove')).toBe(1)
    const action = db.prepare("SELECT * FROM admin_actions WHERE action = 'content_remove'").get()
  ...
  it('TC-CMP-07: 留言下架 → 公开端立即不可见 + 留痕', async () => {
    ...
    expect(actionCount('content_remove')).toBe(1)
    expect(db.prepare("SELECT deleted_by_admin FROM guestbook_messages WHERE id = ?").get(msgId).deleted_by_admin).toBe(1)
  })
  ...
  it('TC-CMP-08: 封禁 → is_banned=1 + 登录拒绝 + token 失效 + 客户端不可见；解封恢复', async () => {
    ...
    expect(dirBefore.json().some(a => a.subdomain === 'cmp-ban')).toBe(true)
    ...
    expect(db.prepare('SELECT is_banned FROM artists WHERE id = ?').get(artist.id).is_banned).toBe(1)
    ...
    expect(dir.json().some(a => a.subdomain === 'cmp-ban')).toBe(false)
    ...
    expect(db.prepare('SELECT is_banned FROM artists WHERE id = ?').get(artist.id).is_banned).toBe(0)
    expect(actionCount('artist_unban')).toBe(1)

    const dirAfter = await app.inject({ method: 'GET', url: '/api/artists' })
    expect(dirAfter.json().some(a => a.subdomain === 'cmp-ban')).toBe(true)
  ...
  it('TC-CMP-09: 不能封禁管理员账号', async () => {
    ...
    expect(db.prepare('SELECT is_banned FROM artists WHERE id = ?').get(admin.id).is_banned).toBe(0)
  })
  ...
  it('TC-CMP-10: 作品发布命中敏感词 → warning，作品照常入库', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) c FROM artworks').get().c).toBe(1)
  })
  ...
  it('TC-CMP-12: 留言命中敏感词 → warning，留言照常入库（pending）', async () => {
    ...
    const row = db.prepare('SELECT status FROM guestbook_messages').get()
    expect(row.status).toBe('pending')
  })
  ...
  it('TC-CMP-13: 主页公告保存命中敏感词 → warning，公告照常保存', async () => {
    ...
    expect(db.prepare('SELECT announcement FROM artists WHERE id = ?').get(artist.id).announcement).toContain('赌博')
  })
