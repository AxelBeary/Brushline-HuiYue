...
describe('audit-a P3-4 自定义单折扣码静默无效', () => {
  ...
  it('TC-P34-01: 自定义单（无计价基准）带折扣码 → VALIDATION + field=discountCode', () => {
    ...
    expect(caught.detail).toMatchObject({ field: 'discountCode' })
    expect(caught.detail.message).toContain('无可计价基准')
...
describe('audit-a P3-7 DELETE 通用问候语存在性', () => {
  ...
  it('TC-P37-02: 删除存在的通用问候语 → success', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) AS c FROM greeting_templates WHERE id = ?').get(id).c).toBe(0)
  })
})
...
describe('audit-a P3-13 transfer 限流配额预耗', () => {
  ...
  function transferRequest(newQq, ip) {
    return app.inject({
      method: 'POST',
...
describe('audit-a P3-15 tracking days 参数化', () => {
  ...
  it('TC-P315-01: 恶意字符串不改变 SQL 结构（参数化安全 + 钳制）', () => {
    expect(() => trackingService.getTrackingSummary('1; DROP TABLE events; --')).not.toThrow()
    expect(() => trackingService.getArtistTrackingSummary(1, '0; DROP TABLE events; --')).not.toThrow()
    // 事件表完好
