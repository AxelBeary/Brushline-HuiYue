...
describe('迁移 v46/v47', () => {
  it('TC-TL-01: client_profiles 表 7 列 + 唯一约束', () => {
    const cols = (db.prepare('PRAGMA table_info(client_profiles)').all() as Array<{ name: string }>).map(c => c.name)
    expect(cols).toEqual(expect.arrayContaining(['id', 'artist_id', 'client_qq', 'tags', 'note', 'created_at', 'updated_at']))
    const applied = db.prepare('SELECT version FROM schema_migrations WHERE version = 46').get() as { version: number } | undefined
    expect(applied?.version).toBe(46)
  })

  it('TC-TL-02: standalone_incomes 表 8 列 + CHECK(amount_cents>0)', () => {
    const cols = (db.prepare('PRAGMA table_info(standalone_incomes)').all() as Array<{ name: string }>).map(c => c.name)
    expect(cols).toEqual(expect.arrayContaining(['id', 'artist_id', 'amount_cents', 'client_name', 'note', 'income_date', 'created_at', 'updated_at']))
    const applied = db.prepare('SELECT version FROM schema_migrations WHERE version = 47').get() as { version: number } | undefined
    expect(applied?.version).toBe(47)
  })

  it('TC-TL-03: 迁移幂等——重跑 initDatabase 不炸且表仍在', () => {
    expect(() => initDatabase(db)).not.toThrow()
    const idx = db.prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND name = 'idx_client_profiles_artist'").get() as { name: string } | undefined
    const idx2 = db.prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND name = 'idx_standalone_incomes_artist_date'").get() as { name: string } | undefined
...
describe('client_profiles CRUD', () => {
  ...
  it('TC-TL-04: upsert 新增 → 再 upsert 更新（不重复插入）', () => {
    ...
    const count = (db.prepare('SELECT COUNT(*) AS c FROM client_profiles').get() as { c: number }).c
    expect(count).toBe(1)
  })
...
describe('tools 路由层（鉴权 + schema + CSV）', () => {
  ...
  it('TC-TL-28: export.csv → BOM + 表头 + 2 行 + Content-Disposition', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.headers['content-type'] as string).toContain('text/csv')
    expect(res.headers['content-disposition'] as string).toContain('income-20260801-20260831.csv')
