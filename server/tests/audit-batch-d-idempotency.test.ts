...
describe('审计批 D-2 幂等键', () => {
  ...
  it('TC-D2-07: 收款同 key 二次提交 → 金额不重复入账、返回首次缓存', async () => {
    ...
    try {
      ...
      expect((db.prepare('SELECT paid_total_cents FROM orders WHERE id = ?').get(orderId) as { paid_total_cents: number }).paid_total_cents).toBe(1000)
    } finally {
      await app.close()
    }
  })
  ...
  it('TC-D2-08: 收款不同 key 正常两笔、无 header 正常一笔', async () => {
    ...
    try {
      ...
      `).run(artist.id)
      const orderId = Number(order.lastInsertRowid)
      const base = { method: 'POST' as const, url: `/api/artist/orders/${orderId}/payments` }
      ...
      expect((db.prepare('SELECT paid_total_cents FROM orders WHERE id = ?').get(orderId) as { paid_total_cents: number }).paid_total_cents).toBe(1800)
    } finally {
      await app.close()
    }
  })
  ...
  it('TC-D2-09: pruneIdempotencyKeys 只清超 24h 的行', () => {
    ...
    expect((db.prepare('SELECT key FROM idempotency_keys WHERE scope = ?').all(scope) as Array<{ key: string }>).map(r => r.key)).toEqual(['fresh'])
  })
  ...
  it('TC-D2-10: GC 接线——启动 GC 自动清超期幂等行（app.ts 定时器同批）', async () => {
    ...
    const app = await buildApp({ logger: false })
    try {
      expect((db.prepare('SELECT key FROM idempotency_keys WHERE scope = ?').all(scope) as Array<{ key: string }>).map(r => r.key)).toEqual(['kept'])
    } finally {
      await app.close()
    }
  })
  ...
  it('TC-D2-11: 手动录单同 key 二次提交 → 返回首次缓存、只落一单（I6-d v75 遗留收尾）', async () => {
    ...
    try {
      ...
      expect(db.prepare('SELECT * FROM orders').all()).toHaveLength(1)
      expect((db.prepare('SELECT source FROM orders').get() as { source: string }).source).toBe('manual')
    } finally {
      await app.close()
    }
  })
  ...
  it('TC-D2-12: 手动录单无 header 正常一笔；不同 key 正常两笔', async () => {
    ...
    try {
      const token = createSession(artist.id, artist.token_version)
      const base = { method: 'POST' as const, url: '/api/artist/orders/manual' }
  ...
  it('TC-D2-13: 损坏 response_json 缓存行被删除并按未命中重执行（不再同 key 永久 500）', () => {
    ...
    ).get(scope, 'bad') as { response_json: string }).response_json).toBe('{"ok":true}')
  })
})
