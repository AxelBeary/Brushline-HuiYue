...
describe('取消 5 秒撤销（815 拍板 #1）', () => {
  ...
  const windowRows = (orderId) =>
    db.prepare('SELECT * FROM cancel_undo_windows WHERE order_id = ? ORDER BY id DESC').all(orderId)
  ...
  it('TC-CU-02: 窗口过期后撤销 → 410', () => {
    ...
    // 订单保持已取消
    expect(db.prepare('SELECT status FROM orders WHERE id = ?').get(order.id).status).toBe('cancelled')
  })
  ...
  it('TC-CU-04: 窗口期内队列位置不动，过期结算后重排', () => {
    ...
    expect(db.prepare('SELECT queue_position FROM orders WHERE id = ?').get(o2.id).queue_position).toBe(2)
    ...
    expect(db.prepare('SELECT queue_position FROM orders WHERE id = ?').get(o2.id).queue_position).toBe(1)
    expect(windowRows(o1.id)[0].consumed).toBe(2)
  })
...
describe('取消撤销路由层（815 拍板 #1）', () => {
  ...
  it('TC-CU-06: POST /cancel 返回 undoWindowMs；/cancel-undo 恢复状态', async () => {
    ...
    // 二次撤销：订单已恢复非 cancelled，状态检查先行拦（400）
