...
describe('F7 发布作品幂等', () => {
  ...
  function seedDeliverable(orderId, name = 'idem.jpg') {
  ...
  it('TC-F7-01: 同一 deliverable 连续发布两次 → 第二次幂等返回同一 artwork，无重复文件/行', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) c FROM artworks WHERE artist_id = ?').get(artist.id).c).toBe(1)
    expect(db.prepare('SELECT COUNT(*) c FROM artworks WHERE source_deliverable_id = ?').get(d.id).c).toBe(1)
  ...
  it('TC-F7-02: 混合请求（已发布 + 未发布）→ 已发布取现有行、未发布正常发布，整体返回', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) c FROM artworks WHERE artist_id = ?').get(artist.id).c).toBe(2)
  ...
  it('TC-F7-03: 并发双发 → 均成功返回同一 artwork，只留一行一文件（不报 500）', async () => {
    ...
    expect(db.prepare('SELECT COUNT(*) c FROM artworks WHERE artist_id = ?').get(artist.id).c).toBe(1)
    expect(db.prepare('SELECT source_deliverable_id FROM artworks WHERE id = ?').get(a[0].id).source_deliverable_id).toBe(d.id)
    expect(imageFileCount()).toBe(1)
  })
