...
function applyMigrationChain(database) {
...
function snapshotSchema(database) {
  ...
  for (const table of tables) {
    snapshot[table] = database.prepare(`PRAGMA table_info(${JSON.stringify(table)})`).all()
      .map(col => ({
        name: col.name,
        type: col.type,
        notnull: col.notnull,
...
function diffSnapshots(a, b) {
  ...
  for (const table of [...tablesA].filter(t => tablesB.has(t))) {
    const colsA = new Map(a[table].map(c => [c.name, c]))
    const colsB = new Map(b[table].map(c => [c.name, c]))
