...
describe('系统增项模板管理 (Admin Addon Templates)', () => {
  ...
  async function createSystemTpl(payload, admin) {
  ...
  it('TC-AT-04: 列表只含系统模板且带引用计数；非管理员 403', async () => {
    ...
    expect(res.statusCode).toBe(200)
    const list = res.json()
    expect(list.every(t => t.artist_id === null)).toBe(true)
    expect(list.some(t => t.name === '画师私有')).toBe(false)
    expect(list.find(t => t.id === sysTpl.id).referenced).toBe(1)
