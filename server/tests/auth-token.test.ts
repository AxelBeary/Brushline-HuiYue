...
import { createSession, verifySession, type SessionPayload } from '../src/features/auth/auth.service.js'

/** SELECT token_version 行 */
...
describe('bumpTokenVersion 令牌版本递增', () => {
  ...
  it('TC-T-01: 连续调用递增 token_version', () => {
    ...
    let row = db.prepare('SELECT token_version FROM artists WHERE id = ?').get(artist.id) as TokenVersionRow
    expect(row.token_version).toBe(2)

    bumpTokenVersion(artist.id)
    row = db.prepare('SELECT token_version FROM artists WHERE id = ?').get(artist.id) as TokenVersionRow
  ...
  it('TC-T-02: bump 后旧 token 版本号不匹配', () => {
    ...
    const session = verifySession(token) as SessionPayload
    expect(session.v).toBe(1)

    // 递增
    bumpTokenVersion(artist.id)
    const fresh = db.prepare('SELECT token_version FROM artists WHERE id = ?').get(artist.id) as TokenVersionRow
  ...
  it('TC-T-04: NULL token_version 兜底为 1 再递增到 2', () => {
    ...
    const row = db.prepare('SELECT token_version FROM artists WHERE id = ?').get(artist.id) as TokenVersionRow
    // COALESCE(NULL, 1) + 1 = 2
    expect(row.token_version).toBe(2)
  })
  ...
  it('TC-T-07: 不影响其他画师的 token_version', () => {
    ...
    const row1 = db.prepare('SELECT token_version FROM artists WHERE id = ?').get(a1.id) as TokenVersionRow
    const row2 = db.prepare('SELECT token_version FROM artists WHERE id = ?').get(a2.id) as TokenVersionRow
