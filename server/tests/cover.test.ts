...
/** Artwork 断言视图（artist.service 内部接口未导出） */
...
describe('封面图 (Cover Artwork)', () => {
  ...
  /** 快速创建作品 */
  async function addArtwork(title: string): Promise<ArtworkView> {
    return (await artistService.createArtwork(artist.id, {
  ...
  it('TC-CV-01: setCover 将作品标记为封面', async () => {
    ...
    const result = artistService.setCover(artist.id, a1.id) as ArtworkView

    expect(result.is_cover).toBe(1)
    // 数据库验证
    const row = db.prepare('SELECT is_cover FROM artworks WHERE id = ?').get(a1.id) as { is_cover: number }
  ...
  it('TC-CV-02: 设多张封面时共存，不互相取消', async () => {
    ...
    // a1 仍是封面（多张共存）
    const r1 = db.prepare('SELECT is_cover FROM artworks WHERE id = ?').get(a1.id) as { is_cover: number }
    expect(r1.is_cover).toBe(1)
    // a2 也是封面
    const r2 = db.prepare('SELECT is_cover FROM artworks WHERE id = ?').get(a2.id) as { is_cover: number }
    expect(r2.is_cover).toBe(1)
    // a3 从未设过
    const r3 = db.prepare('SELECT is_cover FROM artworks WHERE id = ?').get(a3.id) as { is_cover: number }
  ...
  it('TC-CV-03: clearCover 取消封面', async () => {
    const a1 = await addArtwork('作品1')
    artistService.setCover(artist.id, a1.id)
    const result = artistService.clearCover(artist.id, a1.id) as ArtworkView
  ...
  it('TC-CV-05: getArtworks 封面排第一', async () => {
    ...
    const list = artistService.getArtworks(artist.id) as ArtworkView[]
  ...
  it('TC-CV-07: 封面操作不影响其他画师', async () => {
    ...
    // 别人的作品不受影响
    const bRow = db.prepare('SELECT is_cover FROM artworks WHERE id = ?').get(b1.id) as { is_cover: number }
  ...
  it('TC-CV-09: 第 7 张设封面被拦截且保持非封面', async () => {
    ...
    const row = db.prepare('SELECT is_cover FROM artworks WHERE id = ?').get(artworks[6].id) as { is_cover: number }
    expect(row.is_cover).toBe(0)
    // 原有 6 张封面不受影响
    const coverCount = db.prepare('SELECT COUNT(*) AS c FROM artworks WHERE artist_id = ? AND is_cover = 1').get(artist.id) as { c: number }
  ...
  it('TC-CV-10: 取消一张封面后第 7 张可设成功', async () => {
    ...
    const result = artistService.setCover(artist.id, artworks[6].id) as ArtworkView
    expect(result.is_cover).toBe(1)
  })
  ...
  it('TC-CV-11: 已达上限时已封面作品重复设置幂等通过', async () => {
    ...
    const result = artistService.setCover(artist.id, artworks[0].id) as ArtworkView
    expect(result.is_cover).toBe(1)
  })
})
