...
describe('迁移 v40: installments locked/locked_reason 列', () => {
  ...
  function seedInstallment(orderNo: string) {
  ...
  it('TC-MIG-40b: 新插入行 locked 默认 0、locked_reason 默认 NULL', () => {
    ...
    const row = db.prepare('SELECT locked, locked_reason FROM order_payment_installments WHERE order_id = ?').get(orderId) as { locked: number; locked_reason: string | null }
  ...
  it('TC-MIG-40c: 存量行读出默认值 0 而非 NULL（DEFAULT 兼容）', () => {
    ...
    // 模拟存量行迁移后读出
    const rows = db.prepare('SELECT locked FROM order_payment_installments WHERE order_id = ?').all(orderId) as Array<{ locked: number | null }>
