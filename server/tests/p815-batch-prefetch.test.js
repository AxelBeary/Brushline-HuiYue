...
describe('815 P-2 admin 订单分期批量预取', () => {
  ...
  it('TC-P815-03: 多订单多分期 — 每单分期与逐行结果一致且仅 2 条批量查询', async () => {
    ...
    const o1Insts = db.prepare('SELECT id FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order').all(o1.id)
    ...
    const o2Insts = db.prepare('SELECT id FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order').all(o2.id)
    ...
    const byNo = new Map(items.map(o => [o.order_no, o]))
    expect(byNo.get(o1.order_no).installments.map(i => i.status)).toEqual(['paid', 'pending'])
    expect(byNo.get(o2.order_no).installments.map(i => i.status)).toEqual(['paid', 'partial'])
