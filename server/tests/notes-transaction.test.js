...
describe('P2-F10 备注事务', () => {
  ...
  it('TC-NTX-01: addNote 日志写失败 → 备注整体回滚', () => {
    ...
    const notes = db.prepare('SELECT COUNT(*) AS c FROM order_notes WHERE order_id = ?').get(order.id)
    expect(notes.c).toBe(0)
    const logs = db.prepare('SELECT COUNT(*) AS c FROM order_activity_logs WHERE order_id = ?').get(order.id)
  ...
  it('TC-NTX-03: deleteNote 日志写失败 → 备注删除整体回滚', () => {
    ...
    const noteId = db.prepare('SELECT id FROM order_notes WHERE order_id = ?').get(order.id).id
