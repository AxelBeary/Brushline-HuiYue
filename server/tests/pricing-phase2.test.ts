...
// 分期行 / 条目行局部类型
interface InstRow { label: string; amount_cents: number; locked: number; locked_reason: string | null }
interface EntryRow { type: string; delta_cents: number }

// ─── 辅助：四节点工作流（定金10/线稿40/细化30/完稿20，Σbp=100%） ───

function seedFourStageArtist(qq: string = '88201', sub: string = 'p2-four') {
...
function instsOf(orderId: number): InstRow[] {
  return db.prepare(
    'SELECT label, amount_cents, locked, locked_reason FROM order_payment_installments WHERE order_id = ? ORDER BY sort_order ASC'
  ).all(orderId) as InstRow[]
}

function entriesOf(orderId: number): EntryRow[] {
  return db.prepare('SELECT type, delta_cents FROM order_price_entries WHERE order_id = ? ORDER BY id ASC').all(orderId) as EntryRow[]
}
...
describe('R4 锁价规则', () => {
  ...
  function makePricedOrder(artist: ArtistRow, qq: string = '88302'): number {
    const r = db.prepare(`
...
describe('R10 关单后额外项 + R13 done 半终态', () => {
  ...
  it('TC-P2-R13a: done 未付全 + 负增项（减价）→ 冲抵未付节点欠款，refund_item 条目（R13 用户场景②）', () => {
    ...
    const finalPrice = (db.prepare('SELECT final_price_cents FROM orders WHERE id = ?').get(orderId) as { final_price_cents: number | null }).final_price_cents
...
describe('R11 守恒拒绝', () => {
  ...
  it('TC-P2-R11d: 总价列与账本漂移（final_price≠Σ条目）→ checkOrderConservation 抛 LEDGER_DRIFT（815-P2 金额#6）', () => {
    ...
    try {
      ...
      expect(detail.assertion).toBe('LEDGER_DRIFT')
      expect(detail.finalPriceCents).toBe(99999)
      expect(detail.entrySumCents).toBe(30000)
    }
  })
...
describe('端到端主场景（改价→收定金→推进→加价，每步 locked+节点价+守恒）', () => {
  it('TC-P2-E2E: 案例 1 全流程数值推演', () => {
    ...
    const entries = entriesOf(orderId)
    expect(sum(entries.map(e => e.delta_cents))).toBe(50000)
    const order = db.prepare('SELECT paid_total_cents FROM orders WHERE id = ?').get(orderId) as { paid_total_cents: number | null }
