...
/** createOrder 返回行（测试额外访问折扣列） */
type StyleOrder = OrderDetail & { customerToken: string; discount_amount_cents: number | null; discount_code_id: number | null }

/** order_payment_installments 行 */
interface InstallmentRow { label: string; amount_cents: number }

/** order_price_breakdown 行 */
interface BreakdownRow { item_type: string; item_name: string }
...
describe('订单创建画风模式 (createOrder styleSizeId)', () => {
  ...
  it('TC-SO-05: 画风模式 — 生成分期计划', () => {
    ...
    ).all(order.id) as InstallmentRow[]
