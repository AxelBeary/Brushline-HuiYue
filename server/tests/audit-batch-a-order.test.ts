...
import type { ArtistRow, OrderRow, SeededOrder } from './setup.js'
...
function seedOrder(artistId: number, overrides: Record<string, unknown> = {}): SeededOrder {
  const defaults = {
  ...
  const data = { ...defaults, ...overrides } as typeof defaults & { customerToken?: string }
  ...
  const row = db.prepare('SELECT * FROM orders WHERE id = ?').get(Number(result.lastInsertRowid)) as OrderRow
...
describe('audit-a R-7 queue_position 分区重排', () => {
  ...
  it('TC-R7-01: compactQueue 后 formal/buffer 两区各自 1..n 无重复', () => {
    ...
    const formal = db.prepare("SELECT order_no, queue_position FROM orders WHERE artist_id = ? AND queue_zone = 'formal' AND status NOT IN ('delivered','cancelled') ORDER BY queue_position").all(artist.id) as Array<{ order_no: string; queue_position: number }>
    const buffer = db.prepare("SELECT order_no, queue_position FROM orders WHERE artist_id = ? AND queue_zone = 'buffer' AND status NOT IN ('delivered','cancelled') ORDER BY queue_position").all(artist.id) as Array<{ order_no: string; queue_position: number }>
  ...
  it('TC-R7-02: promoteOrder 递补后位置号落在 formal 区末尾且不与任何 formal 单重复', () => {
    ...
    const bufferPos = db.prepare('SELECT queue_position FROM orders WHERE id = ?').get(b2.id) as { queue_position: number }
...
describe('audit-a R-10 createOrder 总价封顶', () => {
  ...
  function seedStyleAndSize(basePrice: number) {
  ...
  function seedOverflowAddon(styleId: number, _unusedMaxQuantity?: number) {
    ...
    return db.prepare('SELECT id FROM style_addons WHERE art_style_id = ?').get(styleId) as { id: number }
  }
