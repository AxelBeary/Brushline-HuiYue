...
import { buildOgMeta, clearOgCache, type OgData } from '../src/features/og/og-meta.service.js'

// M-4 回归：第二参数 host 运行时被忽略（Host 头不再参与 URL 构建）；
// TS 签名为单参，此处断言保留原调用的双参形态（语义零变更）
const buildOgMetaWithHost = buildOgMeta as (subdomain: string, ignoredHost?: string) => OgData
...
describe('M-1 order 域文本字段消毒（sanitizeStoredText）', () => {
  ...
  it('TC-M1-02: addNote 备注恶意内容入库前清理', () => {
    ...
    const note = db.prepare('SELECT content FROM order_notes WHERE order_id = ?').get(order.id) as { content: string }
...
describe('M-2 style 域文本字段消毒（sanitizeStoredText）', () => {
  ...
  it('TC-M2-04: 解绑快照 tpl_name/tpl_unit_label 写入前清理（存量脏数据兜底）', () => {
    ...
    ).get(style.id) as { tpl_name: string; tpl_unit_label: string }
...
describe('M-3 compliance 域文本字段消毒（sanitizeStoredText）', () => {
  ...
  it('TC-M3-01: 举报 description/contact 恶意内容入库前清理', () => {
    ...
    }) as complianceService.ReportRow
  ...
  it('TC-M3-02: 处理留痕 reason 恶意内容入库前清理', () => {
    ...
    const action = db.prepare("SELECT reason FROM admin_actions WHERE action = 'report_resolve'").get() as { reason: string }
...
describe('M-4 OG meta：Host 反射投毒面关闭', () => {
  ...
  it('TC-M4-02: 不设 DOMAIN 时固定降级 localhost，Host 头不参与、回落值不缓存', () => {
    ...
    const first = buildOgMetaWithHost('m4-fallback', 'evil.example')
    ...
    const second = buildOgMetaWithHost('m4-fallback', 'good.example')
  ...
  it('TC-M4-03: 不设 DOMAIN 时默认 OG 同样固定 localhost、不缓存', () => {
    ...
    const first = buildOgMetaWithHost('ghost', 'evil.example')
    expect(first.url).toBe('https://localhost/artist/ghost')

    const second = buildOgMetaWithHost('ghost', 'good.example')
