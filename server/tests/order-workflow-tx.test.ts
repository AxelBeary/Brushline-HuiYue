...
vi.mock('../src/features/order/order.service.js', async (importOriginal) => {
  const mod = await importOriginal<typeof import('../src/features/order/order.service.js')>()
  return {
    ...mod,
...
describe('订单流程事务包裹 (P0-1)', () => {
  ...
  it('TC-TX-01: advanceStage 中间步骤抛错 → 事务回滚，orders/日志均未变更', () => {
    ...
    ).get(order.id) as { current_stage_id: number | null; status: string }
...
describe('v128 修改记录推导 (getRevisionRecords)', () => {
  ...
  it('TC-REV-03 (v129): 「需修改」可反复点击——revision→revision 合法，每点一轮计一次修改', () => {
    ...
    expect((db.prepare('SELECT status FROM orders WHERE id = ?').get(order.id) as { status: string }).status).toBe('done')
  })
})
