...
import { db, cleanDb, seedArtist, seedOrder, type ArtistRow, type OrderRow } from './setup.js'
...
describe('B1 订单响应增强统一 (enrichOrderForArtist)', () => {
  ...
  function makeArtist(qq = '88101', sub = 'enrich-test'): ArtistRow {
  ...
  function makePaidOrder(artist: ArtistRow): OrderRow {
  ...
  function expectEnriched(body: Record<string, unknown>): void {
  ...
  it('TC-EN-03: DELETE extra-items → 响应保持增强字段', async () => {
    ...
    mkdirSync(join(process.env.UPLOAD_DIR as string, 'references'), { recursive: true })
    writeFileSync(join(process.env.UPLOAD_DIR as string, 'references', 'test-a.png'), 'fake-image')
  ...
  it('TC-EN-08: POST/DELETE notes → 响应保持增强字段', async () => {
    ...
    const noteId = addRes.json().notes.find((n: { content: string }) => n.content === '测试备注').id
  ...
  it('TC-EN-09: PUT stage / track-on / stage-back → 响应保持增强字段', async () => {
    ...
    const stages = db.prepare('SELECT * FROM artist_workflow_stages WHERE artist_id = ? ORDER BY sort_order ASC').all(artist.id) as Array<{ id: number }>
