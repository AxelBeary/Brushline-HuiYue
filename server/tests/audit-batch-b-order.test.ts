...
function createReferenceFile(relPath: string) {
...
describe('audit-batch-b P2-12 参考图存在性校验', () => {
  ...
  it('TC-P2-12-02: 客户下单引用真实存在的参考图 → 200 且订单带参考图', async () => {
    ...
    const refRow = db.prepare('SELECT file_path FROM order_references').get() as { file_path: string }
  ...
  it('TC-P2-12-04: 画师追加参考图——不存在被拒 / 真实存在通过', async () => {
    ...
    expect(ok.statusCode).toBe(200)
    expect(ok.json().references.some((r: { file_path: string }) => r.file_path === ref)).toBe(true)
  })
