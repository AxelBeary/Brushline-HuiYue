...
describe('认证服务 (Auth Service) — REQ-027 TOTP', () => {
  ...
  it('TC-A-06: bindTotpInit 重复调用覆盖旧密钥', () => {
    ...
    const row = db.prepare('SELECT totp_secret FROM artists WHERE id = ?').get(artist.id)
  ...
  it('TC-A-09: 错误动态码返回 TOTP_INVALID（含剩余机会提示）', () => {
    ...
    const result = authService.verifyTotpLogin('12345', '000000')
    expect(result.valid).toBe(false)
    expect(result.code).toBe('TOTP_INVALID')
  ...
  it('TC-A-10: 未注册 QQ 防枚举（与码错误同响应）', () => {
    const result = authService.verifyTotpLogin('99999', '123456')
    expect(result.valid).toBe(false)
    expect(result.code).toBe('TOTP_INVALID')
    // 不泄露「未注册」信息
  ...
  it('TC-A-14: 登录成功清零失败计数', () => {
    ...
    authService.verifyTotpLogin('12345', '000000')
    let row = db.prepare('SELECT totp_failed_attempts FROM artists WHERE id = ?').get(artist.id)
    expect(row.totp_failed_attempts).toBe(2)

    // 正确登录
    authService.verifyTotpLogin('12345', computeTotp(secret, Date.now()))
    row = db.prepare('SELECT totp_failed_attempts, totp_locked_until FROM artists WHERE id = ?').get(artist.id)
  ...
  it('TC-A-15: resetTotp 清空绑定（旧密钥失效）', () => {
    ...
    const status = authService.getTotpStatus(db.prepare('SELECT * FROM artists WHERE id = ?').get(artist.id))
    expect(status.hasSecret).toBe(false)
    expect(status.verified).toBe(false)
