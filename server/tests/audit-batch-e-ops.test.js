...
describe('审计批E 运维脚本 (R-6)', () => {
  it('TC-OPS-01: backup-uploads 产出 tar.gz，包含全部文件（含回收站）且内容可读回', async () => {
    ...
    try {
      ...
      const png = entries.find(e => e.name === 'images/1/a.png')
  ...
  it('TC-OPS-03: restore-db 损坏目标库恢复成功，损坏原库留证', () => {
    ...
    try {
      ...
      expect(check.prepare('SELECT v FROM t').get().v).toBe('hello')
      check.close()
    } finally {
  ...
  it('TC-OPS-04: restore-db 校验失败回滚——原库不变、无留证/复制残留', () => {
    ...
    try {
      ...
      const check = new Database(dbPath, { readonly: true })
      expect(check.prepare('SELECT v FROM t').get().v).toBe('original')
      check.close()
