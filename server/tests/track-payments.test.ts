...
describe('track 接口收款明细与截稿日', () => {
  ...
  function makeArtist(qq: string = '88011', sub: string = 'track-pay') {
    return seedArtist({ qq_number: qq, subdomain: sub })
  }

  function addPayment(orderId: number, amountCents: number, note: string, createdAt: string, createdBy: string = 'artist') {
    db.prepare(
  ...
  it('TC-TP-01: track 响应含 payments 数组（金额/备注/时间，按创建时间升序）与 deadline', async () => {
    ...
    // 顺序：created_at 升序
    expect(body.payments.map((p: { amountCents: number }) => p.amountCents)).toEqual([10000, -2000])
