...
describe('审计批 F-1 更新事务回滚', () => {
  ...
  it('TC-F1-07: 事务回滚后 id 序列与计数一致（无幽灵行）', () => {
    const style = styleService.createArtStyle(artist.id, { name: '日系' })
    const before = db.prepare('SELECT COUNT(*) AS c FROM art_styles WHERE artist_id = ?').get(artist.id).c
    try {
      styleService.updateArtStyle(artist.id, style.id, { name: '坏', cover_image: '../x' })
    } catch { /* 预期抛错 */ }
    const after = db.prepare('SELECT COUNT(*) AS c FROM art_styles WHERE artist_id = ?').get(artist.id).c
