...
vi.mock('../src/features/artist/workflow.service.js', async (importOriginal) => {
  const mod = await importOriginal<typeof import('../src/features/artist/workflow.service.js')>()
  return {
    ...mod,
...
describe('audit-a P2-5 createArtist 事务', () => {
  ...
  it('TC-P25-02: seedArtistStages 抛错 → 整个事务回滚（无半建画师）', async () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS c FROM commission_rules').get() as { c: number }).c).toBe(0)
  })
