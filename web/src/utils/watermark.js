 ...
 */
export function wmAnchor(canvasW, canvasH, wmW, wmH, position, margin) {
 ...
 * @param {number} margin 距边距离（px）
 */
export function wmCorners(canvasW, canvasH, wmW, wmH, margin) {
  const m = Math.max(0, margin)
  return [
    { x: m, y: m },
    { x: canvasW - wmW - m, y: m },
 ...
 */
export function wmTileGrid(canvasW, canvasH, wmW, wmH, spacing) {
  const out = []
 ...
 */
export function wmElementSize(ctx, canvasW, { logo = null, text = '', fontSize = 48, logoScale = 0.2 } = {}) {
  if (logo) {
 ...
 */
export function paintWatermarkAt(ctx, { x, y, w, h, text = '', logo = null, opacity = 0.25, rotate = 0 }) {
  if (w <= 0 || h <= 0) return
 ...
 * }} options
 */
export function drawWatermark(ctx, options = {}) {
  const {
    mode = 'corner',
 ...
 */
export async function composeWatermarked(src, options) {
