...
/** 条目合计（分），忽略非法输入 */
export function quoteTotalCents(items: QuoteItem[] | null | undefined): number {
...
/** 文字版报价 labels（标题/客户行/合计行/备注行/平台落款） */
 ...
 */
export function buildQuoteText({ clientName = '', items = [], note = '', labels }: { clientName?: string, items?: QuoteItem[], note?: string, labels: QuoteLabels }): string {
...
export function quoteCanvasHeight(itemCount: number, hasNote: boolean): number {
  const PAD = 48
  const title = 70
  const meta = 34
  const items = 44
 ...
 */
export function renderQuoteCanvas(canvas: HTMLCanvasElement, opts: QuoteCanvasOptions): HTMLCanvasElement {
  const {
    title = '',
    date = '',
    clientLabel = '',
