...
describe('parseMessage 基础结构', () => {
  ...
  it('非字符串/空白输入 → 全空，不抛错', () => {
    expect(parseMessage(null as unknown as string)).toEqual({ clientQq: '', description: '', hints: { amount: null, deadline: null } })
    expect(parseMessage(undefined as unknown as string)).toEqual({ clientQq: '', description: '', hints: { amount: null, deadline: null } })
