...
vi.mock('../storage.js', () => ({
  ...
  safeSetItem: vi.fn<(key: string, value?: string) => void>(),
  safeRemoveItem: vi.fn<(key: string) => void>()
}))

/** beacon/在途请求的事件载荷结构（token + events） */
...
describe('track pagehide 分片（817-D 10-4）', () => {
  ...
  it('积压 120 条：pagehide 分片带走全部未发事件，不再只发前 50 条', async () => {
    ...
    const sendBeaconMock = navigator.sendBeacon as unknown as MockInstance<(url: string, data: Blob) => boolean>
    expect(sendBeaconMock.mock.calls.length).toBeGreaterThanOrEqual(2)
    const bodies: BeaconBody[] = []
    for (const [, blob] of sendBeaconMock.mock.calls) {
    ...
    const inFlight = fetchMock.mock.calls.reduce((sum: number, call) => {
      const body: BeaconBody = JSON.parse(call[1].body)
  ...
  it('buildBeaconBodies：55 条切 50+5 两片，token 每片携带', () => {
    ...
    const bodies = buildBeaconBodies(events as unknown as Parameters<typeof buildBeaconBodies>[0], 'tok').map((s: string): BeaconBody => JSON.parse(s))
