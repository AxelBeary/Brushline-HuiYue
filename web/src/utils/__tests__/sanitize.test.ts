...
/** DOMPurify.sanitize 第二参（本测试关心的白名单字段） */
...
/** afterSanitizeAttributes hook 收到的节点（本测试只用到这两个成员） */
interface HookNode {
  tagName: string
  setAttribute: (name: string, value: string) => void
}
type SanitizeHook = (node: HookNode) => void

const { mockSanitize, mockAddHook } = vi.hoisted(() => ({
  mockSanitize: vi.fn((html: string, _config?: SanitizeConfig): string => html || ''),
  mockAddHook: vi.fn((_name: string, _hook: SanitizeHook): void => {})
}))
