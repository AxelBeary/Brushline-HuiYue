...
describe('parseReorderFill', () => {
  it('空/缺省 → 空集合', () => {
    expect([...parseReorderFill(undefined)]).toEqual([])
...
describe('buildReorderRefs（819-J 二期）', () => {
  ...
  it('排除画师加图（source=artist，客户追踪页不可见，不随单带走）', () => {
    ...
    expect(truncated).toBe(false)
    expect((refs as ReorderRef[]).map(r => r.file_path)).toEqual([
  ...
  it(`超出上限截断为 ${MAX_IMAGE_COUNT} 张并标记 truncated`, () => {
    ...
    expect((refs[0] as ReorderRef).file_path).toBe('references/alice/f0.png')
    expect((refs[refs.length - 1] as ReorderRef).file_path).toBe(`references/alice/f${MAX_IMAGE_COUNT - 1}.png`)
  })
