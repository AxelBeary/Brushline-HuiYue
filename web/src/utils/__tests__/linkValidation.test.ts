...
describe('normalizeLinkUrl（归一化）', () => {
  ...
  it('已有 https:// 原样保留', () => {
    ...
    expect((normalizeLinkUrl('https://weibo.com/testuser') as LinkCheckResult).url).toBe('https://weibo.com/testuser')
  })

  it('http:// 保留', () => {
    expect((normalizeLinkUrl('http://example.com/a') as LinkCheckResult).url).toBe('http://example.com/a')
  })
  ...
  it('空串拒绝', () => {
    ...
    expect(normalizeLinkUrl('   ')).toEqual({ ok: false, reason: LINK_INVALID })
    expect(normalizeLinkUrl(null as unknown as string)).toEqual({ ok: false, reason: LINK_INVALID })
  })

  it('裸域名+端口识别为链接（weibo.com:8080）', () => {
    const res = normalizeLinkUrl('weibo.com:8080/x')
    expect(res.ok).toBe(true)
    expect((res as LinkCheckResult).url).toBe('https://weibo.com:8080/x')
  })
...
describe('validateLink（组合识别）', () => {
  ...
  it('https://space.bilibili.com/123 → 识别平台 2', () => {
    ...
    expect((res as LinkCheckResult).platformId).toBe(2)
  })

  it('未知域名 → platformId null（其他）', () => {
    const res = validateLink('https://myblog.example.net', PLATFORMS)
    expect(res.ok).toBe(true)
    expect((res as LinkCheckResult).platformId).toBe(null)
  })
