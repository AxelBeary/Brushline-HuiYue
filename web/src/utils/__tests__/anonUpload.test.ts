...
/** 上传成功返回体（与 uploadApi.reference 一致） */
...
const h = vi.hoisted(() => ({
  getAnonToken: vi.fn<() => Promise<string | null>>(),
  getFreshAnonToken: vi.fn<() => Promise<string | null>>(),
  reference: vi.fn<(...args: unknown[]) => Promise<UploadedRef>>()
}))
