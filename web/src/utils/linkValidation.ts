 ...
 */
export function normalizeLinkUrl(raw: string): { ok: true, url: string } | { ok: false, reason: string } {
 ...
 */
export function checkLinkLength(url: string): { ok: true } | { ok: false, reason: string } {
  let parsed: URL
  try {
    parsed = new URL(url)
  } catch {
 ...
 * @returns {boolean}
 */
export function matchDomain(hostname: string, matchDomains: string[] | null | undefined): boolean {
 ...
 */
export function validateLink(raw: string, platforms: Array<{ id: number, matchDomains?: string[] | null }> | null | undefined): { ok: true, url: string, platformId: number | null } | { ok: false, reason: string } {
