...
/** 尺寸图宽松形状（profile/gallery 两种口径共用） */
interface SizeLike {
  artwork_image_path?: string | null
  image?: string | null
}

/** 作品行宽松形状（主页 profile 与公开画廊两种口径共用） */
interface ArtistArtworkLike {
...
/** 画师对象宽松形状（只声明本层读取的字段，容忍 hidden 最小形状） */
interface ArtistLike {
  customLinks?: CustomLink[] | null
}

/** useArtistData props（各模板 defineProps 为宽松 Array/Object，此处统一声明） */
interface ArtistDataProps {
  artist?: ArtistLike | null
  tiers?: unknown[] | null
  artworks?: ArtistArtworkLike[] | null
 ...
 */
export function resolveSizeImagePath(size: SizeLike | null | undefined): string {
  if (!size) return ''
 ...
 */
export function buildGalleryFilters(filterSizes: PublicGallerySize[] | null | undefined) {
 ...
 * 没标档位的作品只在「全部」下出现。
 */
export function filterArtworksBySize<T extends ArtistArtworkLike>(artworks: T[] | null | undefined, sizeId: number | null): T[] {
...
export function useArtistData(props: ArtistDataProps) {
  const { t } = useI18n()

  const artist = computed<ArtistLike>(() => props.artist || {})
  const tiers = computed(() => props.tiers || [])
  const artworks = computed<ArtistArtworkLike[]>(() => props.artworks || [])
  const workflowStages = computed(() => props.workflowStages || [])

  /** 统一拼接上传文件 URL，模板不碰路径拼接 */
  const imgUrl = (path: string | null | undefined) => (path ? `/uploads/${path}` : '')

  /** 状态文字走 i18n，模板不写死 */
  const statusText = (status: string | number | null | undefined) =>
    t(`artistHome.status${String(status).charAt(0).toUpperCase()}${String(status).slice(1)}`)

  /** 状态 → Element Plus tag type */
  const statusType = (status: string) => ARTIST_STATUS_TYPE[status] || 'info'
  ...
  const heroArtwork = computed<ArtistArtworkLike | null>(() => coverArtworks.value[0] || artworks.value[0] || null)
