...
describe('resolveSizeImagePath（F3 尺寸图解析）', () => {
  ...
  it('无 artwork_image_path → 回退 image 独立上传路径', () => {
    expect(resolveSizeImagePath({ id: 10, image: 'images/1/solo.png' })).toBe('images/1/solo.png')
  })

  it('两者都无 → 空串（外层兜底画风封面）', () => {
    expect(resolveSizeImagePath({ id: 10 })).toBe('')
    expect(resolveSizeImagePath({ id: 10, artwork_image_path: null, image: null })).toBe('')
  })
})
...
describe('buildGalleryFilters（F6 筛选标签构建）', () => {
  ...
  it('sort_order 缺失 → sortKey 回退 0 不抛错', () => {
    const filters = buildGalleryFilters([{ id: 9, name: 'x', style_id: 1, style_name: '默认' }])
...
describe('filterArtworksBySize（F6 档位筛选）', () => {
  const TAG = (styleSizeId) => ({ style_size_id: styleSizeId, size_name: 'x', style_id: 2, style_name: '默认' })
  const ARTS = [
  ...
  it('选中档位 → 只显示标注该档位的作品', () => {
    expect(filterArtworksBySize(ARTS, 2).map(a => a.id)).toEqual([1, 2])
    expect(filterArtworksBySize(ARTS, 1).map(a => a.id)).toEqual([1])
  })
