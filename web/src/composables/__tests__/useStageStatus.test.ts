...
function stage(id: number, takesPayment = false): StageLike {
  return { id, name: `节点${id}`, takesPayment }
}

/** 典型 4 节点工作流：待确认 → 定金(收款) → 制作 → 尾款(收款) */
function typicalStages(): StageLike[] {
  return [stage(1), stage(2, true), stage(3), stage(4, true)]
}

/** useStageStatus 选项形状（源模块尚为 JS，回调参数显式标注） */
interface StageStatusOptionLike {
  value: string
  disabled: boolean
}
...
describe('useStageStatus', () => {
  ...
  it('无工作流时三个选项全部可用', () => {
    ...
    expect(options.value.every((o: StageStatusOptionLike) => !o.disabled)).toBe(true)
  })

  it('典型工作流：confirmed 和 wip 可用', () => {
    const { options } = useStageStatus(ref(typicalStages()))
    const confirmed = options.value.find((o: StageStatusOptionLike) => o.value === 'confirmed')
    const wip = options.value.find((o: StageStatusOptionLike) => o.value === 'wip')
  ...
  it('2 节点工作流：confirmed 和 wip 禁用', () => {
    ...
    expect(options.value.find((o: StageStatusOptionLike) => o.value === 'confirmed')!.disabled).toBe(true)
    expect(options.value.find((o: StageStatusOptionLike) => o.value === 'wip')!.disabled).toBe(true)
    expect(options.value.find((o: StageStatusOptionLike) => o.value === 'pending')!.disabled).toBe(false)
  })
  ...
  it('当前选项变为不可达时回退到 pending', async () => {
    ...
    expect(options.value.find((o: StageStatusOptionLike) => o.value === 'confirmed')!.disabled).toBe(false)
