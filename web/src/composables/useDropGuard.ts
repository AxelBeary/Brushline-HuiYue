...
export function isSystemFileDrag(event: DragEvent | null | undefined): boolean {
...
export function useDropGuard() {
  ...
  function guardDragEnter(event: DragEvent): boolean {
  ...
  function guardDragOver(event: DragEvent): boolean {
    if (isSystemFileDrag(event)) return true
    event.preventDefault()
    event.stopImmediatePropagation()
    return false
  ...
  function guardDrop(event: DragEvent): boolean {
    if (isSystemFileDrag(event)) return true
    event.preventDefault()
    event.stopImmediatePropagation()
    warnPageDrag()
