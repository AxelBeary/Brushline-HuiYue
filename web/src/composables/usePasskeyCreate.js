...
export function usePasskeyCreate() {
   ...
   * @param {() => Promise<Credential | null | symbol>} ceremony create/get 仪式
   * @param {{ setBusy?: (busy: boolean) => void, onInvalidState?: () => Promise<void> | void }} [options]
   * @returns {Promise<Credential | null>} 成功返回凭据；取消/失败返回 null
   */
  async function passkeyCreateFlow(ceremony, { setBusy, onInvalidState } = {}) {
    setBusy?.(true)
    try {
