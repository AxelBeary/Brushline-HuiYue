...
const msgSuccess = vi.spyOn(ElMessage, 'success').mockImplementation(() => {})
const msgError = vi.spyOn(ElMessage, 'error').mockImplementation(() => {})
...
describe('DiscountCodeManager 启停竞态（G-2）', () => {
  ...
  it('行级启停：在途按钮 disabled，完成后恢复可点', async () => {
    ...
    // 在途：按钮 disabled
    expect(wrapper.vm.codeTogglingIds.has(7)).toBe(true)

    resolveCode({})
    await flushPromises()
    expect(wrapper.vm.codeTogglingIds.has(7)).toBe(false)
