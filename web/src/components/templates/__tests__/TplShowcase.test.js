...
function mountShowcase(props = {}, slots = {}) {
...
describe('TplShowcase tier 形态（档位柜）', () => {
  ...
  it('画师非 open 时选择按钮禁用', () => {
    ...
    expect(wrapper.find('.tpl-tier-select-btn').element.disabled).toBe(true)
  })
})
...
describe('TplShowcase style 形态（画风柜）', () => {
  ...
  it('下单跳转带 styleId/sizeId query；画师非 open 时按钮禁用', async () => {
    ...
    expect(wrapper.find('.tpl-style-order-btn').element.disabled).toBe(true)
