...
vi.mock('vue-i18n', () => ({
  useI18n: () => ({ t: (key, params) => (params ? `${key}:${params.name || ''}` : key) })
}))
...
function mountDrawer(artist = { id: 1, name: 'Alice' }) {
  const wrapper = mount(ArtistDetailDrawer, {
    props: { modelValue: true, artist },
    global: {
      mocks: { $t: (key) => key },
      stubs: EP_STUBS,
      directives: { loading: {} }
...
describe('ArtistDetailDrawer 资料/须知加载失败（P1-B）', () => {
  it('资料失败 → 横幅 + 保存禁用；重试成功 → 恢复', async () => {
    ...
    expect(saveButtons(wrapper)[0].element.disabled).toBe(true)
    ...
    expect(saveButtons(wrapper)[0].element.disabled).toBe(false)
  })
  ...
  it('须知失败 → 横幅 + 保存禁用；重试成功 → 恢复', async () => {
    ...
    expect(rulesSave.element.disabled).toBe(true)
    ...
    expect(wrapper.find('.load-error-banner').exists()).toBe(false)
    expect(saveButtons(wrapper).at(-1).element.disabled).toBe(false)
  })
})
