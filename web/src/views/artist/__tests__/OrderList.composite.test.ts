...
interface OrdersPage {
  items: ReturnType<typeof buildOrder>[]
  total: number
}
...
interface OrderListVM {
  ...
  total: number
  fetchProgress: { total: number; done: number } | null
}
...
async function mountOrderList() {
  ...
  await flushPromises()
  return { wrapper, vm: wrapper.vm as unknown as OrderListVM }
}
...
describe('OrderList 复合筛选缓存 + 进度（REQ-037 批3 D1）', () => {
  ...
  it('a1-4: 复合筛选 totalCount=0 时进度分母不小于 1（防 NaN/Infinity）', async () => {
    ...
    let resolveFirst: ((value: OrdersPage | PromiseLike<OrdersPage>) => void) | undefined
    h.getOrders.mockImplementationOnce(() => new Promise<OrdersPage>((resolve) => { resolveFirst = resolve }))
    ...
    expect(vm.fetchProgress).not.toBeNull()
    expect(vm.fetchProgress!.total).toBeGreaterThanOrEqual(1)
    // 分母兜底后百分比不出现 NaN/Infinity
    expect(Number.isFinite(vm.fetchProgress!.done / vm.fetchProgress!.total)).toBe(true)

    await flushPromises()
    expect(vm.fetchProgress).toBeNull()
  })
  ...
  it('复合筛选 active 仅拉取 wip/pending 等非终态订单（done 视为终态）', async () => {
    ...
    expect(vm.orders.map(o => o.status)).toEqual(['wip', 'pending', 'pending'])
    expect(vm.orders.some(o => o.status === 'done')).toBe(false)
    expect(vm.orders.some(o => o.status === 'delivered')).toBe(false)
