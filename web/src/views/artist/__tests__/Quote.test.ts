...
vi.mock('vue-i18n', () => ({
  useI18n: () => ({ t: (key: string, params?: Record<string, unknown>) => resolveKey(key, params) })
}))
...
describe('报价单生成（812-tools-a）', () => {
  ...
  let originalSecure: boolean | undefined
  ...
  it('一键复制文字版：剪贴板收到模板文本并提示成功', async () => {
    ...
    const text = (navigator.clipboard.writeText as unknown as Mock).mock.calls[0][0]
