...
type SourceOrder = typeof SOURCE_ORDER

const h = vi.hoisted(() => ({
  profile: { subdomain: 'alice' },
  styles: [] as unknown[],
  routeQuery: { from: '806', fill: 'desc,style,note' } as Record<string, string>,
  sourceOrder: null as SourceOrder | null,
  getOrder: vi.fn(() => Promise.resolve(h.sourceOrder)),
  created: null as Record<string, unknown> | null,
  noteCalls: [] as { id: number; data: { content: string } }[],
  deadlineCalls: 0,
  startDateCalls: 0,
  msgSuccess: vi.fn(),
  msgError: vi.fn(),
...
vi.mock('../../../api/index.js', () => ({
  artistApi: {
    ...
    addNote: (id: number, data: { content: string }) => { h.noteCalls.push({ id, data }); return Promise.resolve({}) },
...
const MOCK_STYLES = [
  {
    ...
    sizes: [
      { id: 111, name: '头像', base_price: 80, sort_order: 1, image: null as string | null, image_artwork_id: null as number | null, artwork_image_path: null as string | null, description: null as string | null, work_days: 3, display_status: 'available', addons: [] as unknown[] },
      { id: 112, name: '全身', base_price: 200, sort_order: 2, image: null as string | null, image_artwork_id: null as number | null, artwork_image_path: null as string | null, description: null as string | null, work_days: 7, display_status: 'available', addons: [] as unknown[] }
    ]
  }
]
...
describe('ManualOrder 再来一单预填（818-D）', () => {
  ...
  it('QQ/昵称必带 + 描述/备注/款式尺寸按勾选回填；完成后轻提示', async () => {
    ...
    expect((wrapper.find('input[placeholder="manualOrder.clientQqPlaceholder"]').element as HTMLInputElement).value).toBe('234652462')
    expect((wrapper.find('input[placeholder="manualOrder.clientNamePlaceholder"]').element as HTMLInputElement).value).toBe('老客户')
    expect((wrapper.find('textarea[placeholder="manualOrder.descPlaceholder"]').element as HTMLTextAreaElement).value).toBe('全身立绘 双人')
    expect((wrapper.find('textarea[placeholder="manualOrder.notePlaceholder"]').element as HTMLTextAreaElement).value).toBe('客户喜欢暖色调\n线下已谈好加急')
  ...
  it('未勾选 desc/note：只带 QQ/昵称（fill=style 时仍回填款式尺寸）', async () => {
    ...
    expect((wrapper.find('input[placeholder="manualOrder.clientQqPlaceholder"]').element as HTMLInputElement).value).toBe('234652462')
    expect((wrapper.find('input[placeholder="manualOrder.clientNamePlaceholder"]').element as HTMLInputElement).value).toBe('老客户')
    expect((wrapper.find('textarea[placeholder="manualOrder.descPlaceholder"]').element as HTMLTextAreaElement).value).toBe('')
    expect((wrapper.find('textarea[placeholder="manualOrder.notePlaceholder"]').element as HTMLTextAreaElement).value).toBe('')
  ...
  it('源单读取失败：页内提示错误，不崩溃、不回填', async () => {
    ...
    expect((wrapper.find('input[placeholder="manualOrder.clientQqPlaceholder"]').element as HTMLInputElement).value).toBe('')
