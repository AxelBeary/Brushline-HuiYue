...
interface ManualOrderVM {
  form: { clientQq: string; clientName: string }
}

const h = vi.hoisted(() => ({
  profile: { subdomain: 'alice' },
  created: null as Record<string, unknown> | null,
  createdOptions: null as CreatedOptions | null
}))
...
vi.mock('../../../api/index.js', () => ({
  artistApi: {
    ...
    createManualOrder: (data: Record<string, unknown>, options: CreatedOptions) => {
      h.created = data
...
function draftPayload(clientName: string = 'Remote') {
  return JSON.stringify({
    ...
    styleState: {},
    customAddons: [] as unknown[],
    finalPriceYuan: null as number | null,
    priceTouched: false
  })
}
...
describe('ManualOrder 多标签草稿（G-4）', () => {
  it('storage 事件（他标签页草稿变更）→ 本页未被修改则静默同步表单并落盘', async () => {
    ...
    expect(vm.form.clientQq).toBe('10001')
    expect(vm.form.clientName).toBe('Remote')
  ...
  it('storage 事件 → 本页已被用户修改则不打断（last-edit-wins）', async () => {
    ...
    await flushPromises()
    expect(vm.form.clientName).toBe('Remote1')

    // 用户本地修改
    vm.form.clientName = 'LocalEdit'
    await flushPromises()

    // 他标签页又来一版 → 不覆盖本地正在编辑的内容
    dispatchStorage(DRAFT_KEY, draftPayload('Remote2'))
    await flushPromises()
    expect(vm.form.clientName).toBe('LocalEdit')
    wrapper.unmount()
  })
  ...
  it('清除信号（他标签页提交清草稿）→ 本页重置本地草稿状态（防重复提交）', async () => {
    ...
    await flushPromises()
    expect(vm.form.clientQq).toBe('')
    expect(vm.form.clientName).toBe('')
    wrapper.unmount()
  })
