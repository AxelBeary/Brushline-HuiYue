...
describe('压图改尺寸（812-tools-a）', () => {
  ...
  beforeEach(() => {
    getContextSpy = vi.spyOn(HTMLCanvasElement.prototype, 'getContext').mockReturnValue({ drawImage: vi.fn() })
  ...
  async function chooseImage(wrapper, type = 'image/png') {
  ...
  it('自定义尺寸非法时提示且不压缩', async () => {
    const wrapper = mountResize()
    await chooseImage(wrapper)
    wrapper.vm.preset = 'custom' // script setup 绑定经实例 proxy 可写（dev/test 语义一致）
    await wrapper.find('.ir-process').trigger('click')
    await flushPromises()
