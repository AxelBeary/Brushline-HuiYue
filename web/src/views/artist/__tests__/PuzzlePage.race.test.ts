...
async function mountPuzzle() {
  ...
  await flushPromises()
  return wrapper.vm as unknown as PuzzlePageVM
}
...
describe('PuzzlePage 订单切换守卫（a1-15）', () => {
  ...
  it('快切订单：慢的旧订单响应不覆盖新选中订单', async () => {
    const deferreds: ((value: OrderLite | PromiseLike<OrderLite>) => void)[] = []
    h.getOrder.mockImplementation(() => new Promise<OrderLite>((resolve) => {
      deferreds.push(resolve)
    }))
    const vm = await mountPuzzle()
  ...
  it('旧订单失败（已被新选中取代）不弹错误', async () => {
    const deferreds: Deferred<OrderLite>[] = []
    h.getOrder.mockImplementation(() => new Promise<OrderLite>((resolve, reject) => {
