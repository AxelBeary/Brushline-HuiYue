...
interface ArtworkManageVM {
  artworks: ArtworkRow[]
  covers: ArtworkRow[]
  gridArtworks: ArtworkRow[]
  total: number
  coverCount: number
  remove: (artwork: { id: number }) => Promise<void>
  loadArtworks: () => Promise<void>
  coverOrderOf: (artwork: { id: number }) => number
}
...
async function mountArtwork() {
  ...
  await flushPromises()
  return wrapper.vm as unknown as ArtworkManageVM
}
...
describe('ArtworkManage 删除错误分支 + 分页守卫（a1-12/a1-13）', () => {
  ...
  it('a1-13: 上传刷新与翻页并发时旧响应不覆盖新页', async () => {
    const deferreds: ((value: PagedResult | PromiseLike<PagedResult>) => void)[] = []
    const vm = await mountArtwork()
    h.getArtworksPaged.mockImplementation(() => new Promise<PagedResult>((resolve) => {
    ...
    await flushPromises()
    expect(vm.artworks.map(a => a.id)).toEqual([2])
  ...
  it('b1-B9: covers 封面列表单源（按 cover_order 排序，主图区/计数/序号共用）', async () => {
    ...
    expect(vm.covers.map(a => a.id)).toEqual([3, 1])
    expect(vm.coverCount).toBe(2)
    expect(vm.coverOrderOf({ id: 1 })).toBe(2)
    // 封面只进主图区，网格排除主图
    expect(vm.gridArtworks.map(a => a.id)).toEqual([2])
  })
})
