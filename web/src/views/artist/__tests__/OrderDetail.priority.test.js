...
describe('OrderDetail 优先级快切守卫（a1-3）', () => {
  ...
  it('后发先至：旧响应不得覆盖 prevPriority，最新失败回滚到最新基线', async () => {
    ...
    const deferreds = []
    h.updatePriority.mockImplementation(() => new Promise((resolve, reject) => {
      deferreds.push({ resolve, reject })
    }))

    // 快切：high（seq1）→ low（seq2）
    wrapper.vm.order.priority = 'high'
    const p1 = wrapper.vm.changePriority('high')
    wrapper.vm.order.priority = 'low'
    const p2 = wrapper.vm.changePriority('low')

    // 后发的 low 先成功 → 基线为 low
    deferreds[1].resolve()
    await flushPromises()
    expect(wrapper.vm.prevPriority).toBe('low')
    ...
    wrapper.vm.order.priority = 'high'
    const p3 = wrapper.vm.changePriority('high')
    deferreds[2].reject(new Error('save boom'))
    await flushPromises()
    expect(wrapper.vm.order.priority).toBe('low')
