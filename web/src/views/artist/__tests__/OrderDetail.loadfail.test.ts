...
describe('OrderDetail 首载失败错误态 + 骨架（REQ-037 F1/F2）', () => {
  ...
  it('首载进行中（请求未返回且未失败）：渲染骨架屏，成功后骨架消失', async () => {
    let resolveOrder: ((value: ReturnType<typeof buildOrder> | PromiseLike<ReturnType<typeof buildOrder>>) => void) | undefined
    h.getOrder.mockReturnValueOnce(new Promise<ReturnType<typeof buildOrder>>((resolve) => { resolveOrder = resolve }))
    const wrapper = await mountDetail(null)
