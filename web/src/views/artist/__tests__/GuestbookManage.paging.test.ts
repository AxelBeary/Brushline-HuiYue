...
interface GuestbookManageVM {
  messages: GuestbookMessage[]
  filteredMessages: GuestbookMessage[]
  pagedMessages: GuestbookMessage[]
  page: number
  statusFilter: string
  loading: boolean
  onFilterChange: () => void
...
function mountPage() {
  const wrapper = mount(GuestbookManage, {
    global: {
      mocks: {
  ...
  return { wrapper, vm: wrapper.vm as unknown as GuestbookManageVM }
}
...
describe('GuestbookManage 分页消费（G-8）', () => {
  it('按 { items } 消费并循环拉取全部分页（pageSize=100）', async () => {
    ...
    h.getMessages.mockImplementation(({ page }: { page: number }) => {
    ...
    expect(vm.messages).toHaveLength(120)
    expect(vm.filteredMessages).toHaveLength(120)
    expect(vm.pagedMessages).toHaveLength(20) // 本地分页默认 20/页
  })
  ...
  it('状态筛选：按当前页内数据过滤，筛选切换重置页码', async () => {
    ...
    expect(vm.filteredMessages.every(m => m.status === 'pending')).toBe(true)
    // 45 条中 pending 约 15 条 → 不足一页仍展示
    expect(vm.filteredMessages.length).toBeGreaterThan(0)
  })
  ...
  it('翻页：pagedMessages 按页码切片', async () => {
    ...
    const { vm } = mountPage()
    await flushPromises()
    expect(vm.pagedMessages.map(m => m.id)).toEqual(all.slice(0, 20).map(m => m.id))

    vm.page = 3
    await flushPromises()
    expect(vm.pagedMessages.map(m => m.id)).toEqual(all.slice(40, 45).map(m => m.id))
  })
