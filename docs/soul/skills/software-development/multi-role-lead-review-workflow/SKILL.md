...
**完整设计书 7 章结构**（`docs/画师工作台视觉提案-v2.html`）：
...
6. **STATUS 待办池时效性**：读 `STATUS.md 待办池`，对照 `git log` 检查标"🔵 修复中"的条目是否已合入 master。高频过时：上一版本收工时忘了更新此文件（如 P1/PERF-1 标"修复中"但 v0.24 已合入）。发现过时条目记下来，收尾时统一改 ✅。
