...
- **切了分支 = 当轮必须写完代码 + commit。** 不允许"切了分支、读了文件、然后停了"。如果中途被打断（用户新消息/新任务），恢复后第一件事是检查有没有半成品分支（`git log origin/master..HEAD` 为空 = 没 commit = 事故）。
- **api/index.js 是隐含授权文件。** 任何涉及新接口调用的任务，api/index.js 自动在授权范围内（项目惯例：所有 API 调用走此文件）。不需要每次向一号确认，但 comms 里注明。
- **契约稳定即可并行。** 后端未合入 master 但契约已定（SPEC 文档 + 一号指令文件明确了字段名/端点/响应格式）时，前端对着契约实施，不等后端。改动必须向后兼容（字段缺失时回退旧行为）。
