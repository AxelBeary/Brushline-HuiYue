...
**按受众分，不按技术层分。** 二号负责客户端全部前端（4 模板 + OrderForm + TrackOrder + 共享 Tpl* 组件），三号负责画师后台 + 管理后台的全部前后端。

- 画师后台页面（Dashboard、Settings、QueueBoard 等）和管理后台页面（AdminDashboard、HealthCheck 等）归三号，不在二号职责内。
- 共享组件（`web/src/components/shared/Tpl*.vue`）归二号所有，三号只消费不修改。
- 二号专注客户端视觉一致性和 4 模板差异化，不被画师/管理后台分散精力。
