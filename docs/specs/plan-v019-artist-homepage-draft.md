...
> **作者**：四号（需求整理）
> **日期**：2026-08-01
> **状态**：用户已确认方向（Q1~Q5 全部拍板）+ 排期已定稿（2026-08-01），转交一号审核
...
| # | 项 | 工时 | 负责 |
|---|---|---|---|
| F2 | 瀑布流 | 2h | 二号（纯前端，0 依赖，最先开工） |
| F3 | 小公告 | 3h | 三号后端 1h → 二号前端 1.5h → 测试 0.5h |
| F1 | 点赞 | 4h | 三号后端 1h → 二号前端 2.5h → 测试 0.5h |
| F4 | 留言板 | 9h | 三号后端 3h → 二号客户端 2h + 画师端 2h + 管理端 1h → 测试 1h |
| HC | 系统自检 | ~6.5h | 三号后端 3h → 二号前端 2.5h → 测试 1h |
| S2 | SPEC-002 §八（4 个拍板项） | 待估 | 三号/二号 |
| S5 | SPEC-005 额度池 | 待估 | 三号出设计 |
| P1-4 | 参考图 tooltip | ~15min | 二号（小修，穿插） |

### 9.2 优先级与并行策略

**优先级**：画师主页（F2→F3→F1→F4）> 系统自检（穿插）> SPEC-002 §八 / SPEC-005

**并行原则**：不阻塞多人同时开工。

| 阶段 | 三号（后端） | 二号（前端） | 说明 |
|------|-------------|-------------|------|
| 第 1 波 | F3 公告后端 + F1 点赞后端 | **F2 瀑布流**（纯前端，0 依赖） | 二号不等任何人，立刻开工 |
| 第 2 波 | F4 留言板后端（3h） | F3 公告前端 + F1 点赞前端 | 三号做留言板时二号消化第 1 波产出 |
| 第 3 波 | HC 系统自检后端 / S2 / S5 | F4 留言板前端（客户+画师+管理） | 留言板前端排最后，填二号后半段 |
| 收尾 | 补测试 / S5 设计 | 4 模板统一回归 + P1-4 tooltip | 一轮回归覆盖全部新功能 |

### 9.3 留言板决策

用户拍板（2026-08-01）：**留言板进 v0.19，不推后。**

四号研判理由（用户认可）：
1. 不阻塞并行——三号后端排后面，二号前端排后面，各走各的
2. 4 模板适配只做一轮——点赞/公告/留言板一批嵌完，避免二次熟悉成本
3. 9h 分散在 3 端 + 后端 + 测试，不是单人扛

### 9.4 下一步

1. 一号审核排期，确认并行策略可行
2. 三号确认 T1/T2（点赞数据模型 + 留言板限流），评估实际工时
3. 二号确认 T3/T5（公告位置 + 点赞 0 计数显示），评估实际工时
4. 四号根据反馈更新正式 spec（如需要）
