...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| POST | /api/artist/orders/:id/payments | requireAuth + requireOwn | 记录一笔收款（amount_cents + note） |
| DELETE | /api/artist/orders/:id/payments/:paymentId | requireAuth + requireOwn | 撤销一笔收款（或记负数，待三号定） |
| GET | /api/artist/orders/:id/payments | requireAuth + requireOwn | 收款流水列表 |
...
简化为一行进度，不暴露分期细节（客户不需要知道画师的工作流节点）。

**待用户拍板**：客户端是否仍显示分期明细？还是只显示总额进度？
...
6. 当附加项增加 ¥50 时，应收总额应该 +¥50，待收自动重算
7. 当客户访问进度页时，应该看到"已付 ¥X / 应付 ¥Y"+ 进度条
8. 当管理员查看订单时，应该能看到完整收款流水
9. 当老订单（迁移前已有已付分期）加载时，paid_total_cents 应该等于原已付分期的 SUM
10. 当工作流节点推进时，不应该改变收款状态（推进和收款解耦）
...
| # | 问题 | 选项 | 建议 |
|---|------|------|------|
| Q1 | 分期节点是否需要"部分付款"状态？ | A 三态（paid/partial/pending）/ B 两态（covered/uncovered） | A（更精确，客户付了一半能看到） |
| Q2 | 客户端是否显示分期明细？ | A 只显示总额进度 / B 显示总额+分期参考 | A（客户不需要知道画师内部节点） |
| Q3 | 撤销收款的方式？ | A 删除记录 / B 记负数流水 | B（保留审计痕迹） |
| Q4 | 多收的钱怎么处理？ | A 仅显示"多收 ¥X" / B 支持退款记录 | A（v1 够用，退款线下处理） |
