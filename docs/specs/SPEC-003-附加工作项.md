...
**理由**：
- 约稿场景下画师是服务方，追加需求通常由客户口头提出、画师录入
- 做确认流程 = 客户需要登录平台点确认 → 增加摩擦 → 画师嫌麻烦不用
- 纠纷场景下系统备注 + 时间戳已提供审计轨迹

### v2 候选（不在本次范围）

- 客户确认流程：附加项添加后状态为 `pending`，客户在进度页点"同意"后生效
- 需要：`order_extra_items.status` 列 + 客户端确认接口 + 通知机制
- 触发条件：用户实际使用中反馈"需要客户确认"
