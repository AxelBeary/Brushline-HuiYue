...
1. **映射规则**：第一个节点=pending、中间=wip、最后=done，这个映射是否符合你的使用习惯？还是你希望每个节点都能自定义映射到哪个状态？
2. **回退语义**：打回修改时状态显示"revision"还是直接显示回退到的节点名？
3. **新订单默认接入**：新建订单是否自动接入工作流（current_stage_id=第一个节点）？还是画师手动选择"启用流程跟踪"？
4. **客户可见度**：客户看到的是节点名（"线稿确认"）还是进度百分比（"4/7"）？还是两者都要？
