...
> 最后更新：2026-08-08 v43（v0.44 收编：REQ-035 工具类目收尾 + 05B 滑块/暗色遮罩 + 纪律性审计 + 时间条核查）——master `7c81d48` 与 origin 同步。**npm 版本已对齐 0.44.0**（server+web+lock，双轨一致）。**容器待重建**（等 05C 审计修复批合入后一次性重建收全，避免重建两次）——当前容器 = v0.43 时代（`6f0c0d9`）。**AUTH_DEV_MODE=false（正确状态）**。
