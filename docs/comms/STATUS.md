...
> 🔑 **新会话接手指南**：无在途施工。v135〜v139 已合入 master（63363bc6）并进本机生产（部署全绿）；开放项：用户公网跑 update.sh 同步后复验六项（账号与安全页日历订阅/时间条全年档与仅进行中过滤与临期藤黄色/价目卡导入与双布局/统计导出页收入趋势图/小票打印机导 PNG）+ v134 遗留（公网 Passkey 重注册）；吸纳缓行项留拍板：订单模板库/着色模式五维/仪表盘拖拽布局/时薪统计（均需订单工时字段或交互骨架改动，各自独立拍板）。
