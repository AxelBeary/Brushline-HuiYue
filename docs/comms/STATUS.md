...
---
## 当前阶段：v0.37 轮全部合入——REQ-025 引擎第一阶段就位，待用户终验 v0.36 + 拍板第二阶段

| 角色 | worktree/分支 | 内容 | 状态 |
|------|--------------|------|------|
| 二号 | — | v0.36 六路回归实测：**六路全过零 bug**（一号查库对账验证数据还原属实） | ✅ 报告已收 |
| 三号 | —（worktree 已回收） | REQ-025 计价引擎第一阶段（7 纯函数 + 49 测试 + 迁移 v39） | ✅ 已合入 `aa6891e`（server 769/769，容器重建 + v39 实跑验证通过） |
| 四号 | — | REQ-025 文档轮：R13 done 半终态 + 第二阶段草案 | ✅ 已合入 `32add0e` |
| 五号 | — | 价格守卫批：delivered/cancelled 后端守卫（**done 不拦**） | ✅ 已合入 `8c38032` |
