...
> 最后更新：2026-08-05 终验反馈轮六路全部合入 master（B1/H1/F1/P批/T批/G1G2）——v0.36 波 2 修复收口，等用户终验
> 维护者：一号（主理人）
> **刷新后自包含**：新会话只读本文件即可完全恢复，不依赖任何对话记忆。
---
## master 状态

- **HEAD**：`c3cabd0`（G1/G2 合入，六路收口），与 origin 同步
- **测试基线**：server 713/713 · web 166/166 · tsc 0 · eslint 0 · build ✓
- **容器**：六路合入后重建，带入全部修复（含 B1 enrich 覆盖的 deleteNote 缺 currentStageId 问题）
- **备份**：`data/commission.db.bak-v036-verify`
- **迁移**：v38
...
| 二号-B | — | ✅ **G1/G2 已合入**（`91c1cc6`→merge master 重跑 166/166→`c3cabd0`） | 页内拖拽守卫（7 处接入）+ 手动录单价格脏标记（005 根因）；一号审 diff + 合并后实测 | 低/中 |
