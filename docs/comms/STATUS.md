...
- **迁移**：v42（F2 社交平台表）为最新
- **comms**：STATUS + 五号两份核实报告（安全审计/前端优化，待排期消费）；五号视觉巡检批用户拍板取消（2026-08-06，派工文件已删）；二号手动录单日期批派工/交付已消费删除（合入即删）
...
**在途情况（2026-08-06 刷新）**：五号视觉巡检批用户拍板取消（派工文件已删）；二号手动录单日期批已合入（`9815a3c`）。二三四五号均待派。
