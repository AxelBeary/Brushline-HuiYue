...
> 最后更新：2026-08-05 v0.37 轮收官——TOTP 上线门槛解除，全员空闲；**下一动作：派工 v0.38 画师后台视觉重设计（开工草案已备，见下）**
> 维护者：一号（主理人）
...
- **HEAD**：`58344fe`（v0.37 收官清理），与 origin 同步
- **工作树**：干净，worktree 只剩主仓
...
---
## ⬇️ 下一步：v0.38 画师后台视觉重设计——开工派工草案（新会话照此派工）

**用户已拍板**：REQ-026 Q1「一次性全量切换」（不留双轨，token 层建成后全页面统一套用）。需求稿 `docs/requirements/REQ-026-画师后台视觉重设计.md` 已落库，设计依据三份文档：`docs/画师工作台视觉提案-v2.html`（视觉基准）+ `docs/画师后台视觉规范-v1.md` + `docs/design-brief-画师后台视觉重设计.md`。

**总原则（最高约束，写进每份派工）**：外部设计提案 = 视觉语言参考，不是功能规格。功能以现有系统 + 各 REQ 为准——提案里的新功能形态（⌘K 命令面板、通知中心铃铛）一律不进本批；视觉与现有功能冲突时功能交互不变、只换视觉皮肤。客户端 4 模板完全不动。

**建议拆法（二号为主力，纯前端视觉批，量大风险低）**：
- **第一批（骨架 + 核心）**：token 体系（CSS 变量双主题）+ 侧栏/顶栏骨架 + 主题切换 → 仪表盘 / 排期看板 / 订单详情（含开稿/截稿日二合一日期卡）/ 手动录单
- **第二批（其余页面按 token 推导）**：订单管理 / 开稿管理 / 价格管理 / 作品管理 / 留言管理 / 设置 / 登录 + 管理后台（低频，只套 token 与基础组件）
- 每批独立 worktree 交付、一号审核（逐页对照 REQ-026 §六验收 10 条 + 浏览器实测截图）

**一号审核要点**：7 色语义一对一不破（花青=进行中/朱砂=逾期/石绿=完成/藤黄=待确认/赭石=客户）；统计数字墨色不上色；文楷只落标题/数字/印章；客户端零影响（验收 10）；任何阶段不允许两套主题 token 并存。

**开工前动作**：`git worktree add ../artist-commission-v038 -b v038-visual master` 建 worktree → 写派工 `docs/comms/01-to-02-v038视觉重设计第一批-2026XXXX.md` → 转用户下达开工指令。
