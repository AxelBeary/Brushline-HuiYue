...
---
## 当前阶段：v0.37 第二阶段收尾——A/B/C/D 核心全合入，D 路补漏批待派，四号需求深聊批已落库

| 角色 | worktree/分支 | 内容 | 状态 |
|------|--------------|------|------|
| 三号 | artist-commission-totp / v037-totp | **REQ-027 TOTP 动态口令登录（P0，唯一上线门槛）**：TOTP 核心 + 绑定/登录/重置/CLI + 旧登录码一刀切移除 + 迁移 v41 | 🔄 已派工 |
| 二号 | artist-commission-manualstyle2 / v037-manual-style-fix | D 路补漏批：REQ-029 用户拍板缺口（R2 提示 / R5 自定义增项 / R6 图片开关 / B2 取消选中） | 🔄 已派工 |
| 五号 | — | C 路：demo-data 切流 | ✅ 已合入 `eb0b4d0`（空闲，待下轮） |
| 四号 | — | 需求深聊批：REQ-026/027/028/029/030 + REQ-022 回写 + 传播修改 | ✅ 已落库 `13fa28d`+`13c5c57`（空闲） |
