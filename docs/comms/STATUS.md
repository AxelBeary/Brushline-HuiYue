...
| **v0.35（最后功能版）** | REQ-024 画风档位统一 **F1-F6 全量**（档位带图/后台合并+多画风开关/点档位切大图/下单预选/旧模型迁移 v37/**作品档位标注+画廊筛选 F6**） | 需求全文审核通过，待 v0.34 合入后派工 |
...
- **波 1（三号）**：v37 迁移一次建全——style_sizes 加图/描述/天数字段、artists 加多画风开关字段、artworks↔style_sizes 多对多关联表（F1 从作品集挑图 + F6 档位标注共用，四号建议采纳，避免二次迁移）、artworks 加自由描述字段；+ 画风带图 API + 后台「画风与价格」合并入口 + 多画风开关 + F5 旧模型迁移脚本（showcase 直接丢弃）
- **波 2（二号）**：客户端展示柜 F3 点档位切大图 + F4 下单预选（在 v0.34 任务 B 基础上扩展入口区分与跳第三步）+ F6 画廊筛选标签 + 大图档位标签跳下单
