...
> 最后更新：2026-08-05 11:20——执行模式切换为**用户外部窗口开工**（可观察可叫停），一号会话只做门禁。三路接力指令在 `docs/comms/01-派工-用户侧接力指令-20260805.md`。REQ-022 决策 4/5 项已拍板（F1×3 + F2 工时；F5 未读语义 a/b/c 待拍）
...
- **HEAD**：`5049aff`（五号 F5 留言筛选合入），与 origin 同步
- **工作树**：主仓 + 两个并行 worktree（v038 视觉 / f2 外链后端）；f5 worktree 已回收
- **测试基线**：server 845/845（50 文件）· web 192/192（12 文件）· tsc 0 · lint 0
...
10. **⚠️ 管理员 TOTP 绑定待办（紧急）**：查实管理员账号 QQ 10003 `totp_verified=0`（未绑定）。旧登录码已删，当前仅靠旧 cookie 会话撑着，cookie 一过期即被锁死门外。须在 cookie 有效期内于管理后台完成绑定，或由一号协助直写 DB 绑定。
...
| 角色 | 状态 |
|------|------|
| 二号 | **v038 视觉批第四轮待开工**（用户外部窗口；断点 `158250f` 已锁定：剩 OrderDetail 日期卡/ManualOrder/Preferences/截图验收） |
| 三号 | ✅ addons 评估已合入（`6f79294`）→ **F2 外链后端已交付通过审核**（分支 `f2-social-backend` HEAD `4946993`，877/877 独立复跑全绿，防投毒向量 17 类全过；**按门控暂不合入**，等 v0.38 后 F2 前端批一起合） |
| 四号 | 空闲（拆解批已合入 `393779f`；F1 派工等 v0.38 合入后） |
| 五号 | ✅ F4（`131dee0`）+ F3（`1db4f96`）+ F5 留言筛选（`5049aff`）已合入 → **文档同步批待开工**（REQ-027 TOTP 后的 docs 漂移修复，授权仅 docs/，低风险） |
