...
> 最后更新：2026-08-08 v31（五号 hover 批合入）——**hover 鬼畜修复批已合入**（master `d3c3c18`，16 处+自查 6 处，web 215/215 + lint 0 + build 过 + 复扫 0 命中）。五号空闲，w5 已清理。**四路并行剩三路执行中**：三号 REQ-035 后端包（w3 `beta/req035-backend`）+ 二号管理后台重设计（w2 `beta/admin-redesign`）+ 四号 repowiki 外部 13 处（w4 `beta/repowiki-external-13`，HEAD 已推进）。master `d3c3c18` 同步；**容器落后 v0.43**（web 镜像 08-07 23:46 = v0.41 时代，布局批+菜谱批合入其后，待收编重建）。**REQ-035 前端批（基础四件套/批A前端/批B/批C前端/批D/批E/批F-F2-G）待 hover 批合入后派**——**现在 hover 已合入，可以拆派**（ArtistLayout/OrderDetail/QueueBoardCalendar 已清场；五号待派 F-F2-G 三连，二号待派基础四件套等）。
...
| 角色 | 状态 |
|------|------|
| 二号 | 🔄 **执行中：管理后台重设计**（w2 `beta/admin-redesign`，派工 `02-派工-管理后台重设计`）→ 合入后待派 REQ-035 前端批（基础四件套等，**hover 已合入可拆派**） |
| 三号 | 🔄 **执行中：REQ-035 后端包**（w3 `beta/req035-backend`，派工 `03-派工-REQ035后端包-20260808`）→ 合入后待派：REQ-035 批A前端数据支撑（若需补充端点） |
| 四号 | 🔄 **执行中：repowiki 外部 13 处**（w4 `beta/repowiki-external-13`，HEAD 已推进 `390a40d`，接近交付） |
| 五号 | ✅ **hover 鬼畜修复批已合入**（`d3c3c18`，16处+自查6处，门禁全过）→ **待派：REQ-035 批F→F2→G 时间条/月历三连**（QueueBoardCalendar.vue 同文件串行，hover 已合入可开工） |
