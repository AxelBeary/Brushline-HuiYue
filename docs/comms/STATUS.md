...
| 角色 | 状态 |
|------|------|
| 二号 | ✅ 批3查单增强已合入（`38b5c5b`：A1我的订单/U1需求回顾前端v-if守卫/换色埋点，215/215）→ 待派 |
| 三号 | ✅ 批3 track字段已合入（`38b5c5b`：description+references client-only，930/930）→ 待派 |
| 四号 | ✅ 批2 OD拍板已合入（`695ef93`）+ ✅ 批3 REQ-033埋点口径已合入（`38b5c5b`，含拍板：端点凭证选B+漏斗命名沿用第三方）→ 待派 |
| 五号 | ✅ 批1模板已合入（`544367d`）+ 🔄 批3视觉OD进行中（w7） |
