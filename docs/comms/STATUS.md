...
> 最后更新：2026-08-08 v30（一号刷新后批量派工）——**四路并行已派出**：三号 REQ-035 后端包（w3 `beta/req035-backend`：client_profiles+standalone_incomes+A1导出）+ 五号 hover 鬼畜修复批（w5 `beta/hover-jitter-fix`，16处+自查跳过admin/）+ 二号管理后台重设计（w2 `beta/admin-redesign`，AdminLayout已存在改重构）+ 四号 repowiki 外部 13 处（w4 `beta/repowiki-external-13`）。master `8fd9015` 同步；8 个已合入遗留分支已清理；**容器落后 v0.43**（web 镜像 08-07 23:46 = v0.41 时代，布局批 00:17 + 菜谱批 00:21 合入其后，待 v0.43 收编重建）。**REQ-035 前端批（基础四件套/批A前端/批B/批C前端/批D/批E/批F-F2-G）待 hover 批合入后派**（动 ArtistLayout/OrderDetail/QueueBoardCalendar 同文件冲突）。
...
> 已拍板待做：动画修复批 + UI 优化批（第三方方案已核实）+ 画师统计页前端 + 埋点三态前端 + 作品分页前端 + **v0.43 容器重建**（容器落后布局批+菜谱批）；**REQ-035 前端批**（基础四件套 A1导出/A2收据/B1分享/C4时区 + 批A前端 + 批B跨平台解析 + 批C前端散单页 + 批D今天吃什么页/水印 + 批E拼图/排期公示 + 批F-F2-G时间条/月历三连）**待 hover 批合入后派**（同文件冲突：ArtistLayout/OrderDetail/QueueBoardCalendar）
...
- **HEAD**：`ed0ef4d`（STATUS v29），与 origin 同步
- **工作树**：主仓干净（8 个已合入遗留分支已清理 2026-08-08）
...
- **版本**：业务 v0.43 巡检收尾（布局批 A1-A8 + 今天吃什么菜谱 124 条已合入）↔ npm 0.43.0；v0.44 规划 = 工具类目 + 时间条/月历 + 收编（REQ-035）
- **容器**：⚠️ **落后 v0.43**——web 镜像 08-07 23:46 = v0.41 时代（`ba546a4`），v0.43 布局批（00:17）+ 菜谱批（00:21）合入其后，前端内容未生效。备份 `bak-pre-v041-rebuild-2026-08-07T12-52-29-186Z`。AUTH_DEV_MODE=false（正确状态，STATUS 已核实）
...
- **comms**：STATUS v29 + REQ-035 定稿转交 + hover 鬼畜派工 + 管理后台重设计派工 + 各派工（已消费的合入即删）
...
| 角色 | 状态 |
|------|------|
| 二号 | 🔄 **执行中：管理后台重设计**（w2 `beta/admin-redesign`，派工 `02-派工-管理后台重设计`）→ 合入后待派 REQ-035 前端批（基础四件套等，需等 hover 批合入清场） |
| 三号 | 🔄 **执行中：REQ-035 后端包**（w3 `beta/req035-backend`，派工 `03-派工-REQ035后端包-20260808`）→ 合入后待派：REQ-035 批A前端数据支撑（若需补充端点） |
| 四号 | 🔄 **执行中：repowiki 外部 13 处**（w4 `beta/repowiki-external-13`，派工 `04-派工-repowiki外部13处-20260808`） |
| 五号 | 🔄 **执行中：hover 鬼畜修复批**（w5 `beta/hover-jitter-fix`，派工 `05-派工-hover鬼畜修复`，16处+自查跳过admin/）→ 合入后待派：REQ-035 批F→F2→G 时间条/月历三连（QueueBoardCalendar.vue 同文件串行） |
