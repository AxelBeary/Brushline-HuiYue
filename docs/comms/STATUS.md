...
> 最后更新：2026-08-08 v43（v0.44 收编：REQ-035 工具类目收尾 + 05B 滑块/暗色遮罩 + 纪律性审计 + 时间条核查）——master `b926c74` 与 origin 同步。**npm 版本已对齐 0.44.0**（server+web+lock，双轨一致）。**容器已重建 = master `b926c74`**（v0.44 全部批次上线：批A客户视图/批D水印/批E拼图/track补字段/05B滑块/时间条核查/审计报告）。备份 `commission.db.bak-pre-v044-2026-08-08T222500Z`（如有）。**AUTH_DEV_MODE=false（正确状态）**。
