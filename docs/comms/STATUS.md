...
> 📦 **基线**：server 1367 / web 438 / E2E 11；accept-baseline.json 已同步。master 门禁合并态实测全绿（lint 0 错 1 已知警告/check-i18n/vitest 438）。
...
> ✅ **围剿 P0 全部修完**（2026-08-13，逐条核实后修，均有门禁验证）：①b4 OrderForm loadRetry 键误引命名空间致渲染键名（9231ca1）②a1 TOTP 重绑 Passkey 链全断：注册仪式改登录仪式防误注新凭据+confirm 补带 credential 否则必 400+rebind-init 不发无效挑战（95a13b3）③a2 移动端底部 CTA 尺寸步补校验防提交 styleSizeId=null（2321a6f）④t1 稿价计算器移除 v50 退役 multiplier UI/请求字段否则每次算价必 400（9c80df6）⑤d2 setup 劫持窗口堵死（admin_qq 已写即拒重建）+TC-SETUP-09b（e90b585）⑥d3 v50 重建迁移 4 表补崩溃恢复防数据丢失（4fad9f6，v49/v38 同模式记 P1）⑦b1 报价单日期 UTC 改本地（东八区凌晨必差一天，040947b）。
> 🔜 **围剿剩余**：b5 键盘不可达热区（a11y P0 组，排波）；各路 P1 汇总排修复波（约 90 条）；P2 汇总给用户（~180 条）。**17 路猎杀全收割**：T2 死码 P1 可安全删 41 项+P2 30（console.log 定性/依赖取舍）；V1 部署增强清单（冒烟深度/回滚/备份验证/海外迁移差异）；V2 文档 35 条（含画师使用说明书与新看板矛盾需重写）；V3 注释+文案 57 项（P1×19）。日志 workspace/temp/hunt-*-codex.log。
