...
> 最后更新：2026-08-08 v37（二号管理后台导航分组合入）——**管理后台导航分组已合入**（master `9193b9d`：AdminLayout 左侧导航分组 概览/运营/配置与监控，分组标题渲染 + 折叠态隐藏组标题 + 移动端抽屉分组 + i18n 3键，3 文件 +58/-27，门禁全过 + 3 截图）。**全部角色空闲**，w2 已清理。master `9193b9d` 同步。**容器落后 v0.43** 待收编重建。
