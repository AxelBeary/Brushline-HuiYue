...
> 最后更新：2026-08-09 v51（**批6 交互与布局改造完成，容器已重建含新 UI，待用户复验**）——master 与 origin 同步。
> ✅ **批6 内容（用户逐项拍板需求）**：①尺寸行右组（三态+预览/编辑/删除）成组右对齐，窄屏换行不错位（VL 1024 实测通过）；②三态选中态色块填充提可见度；③增项池改单块三行（普通增项/用途/加急，类别色标签）；④默认徽标仅多画风关闭时显示；⑤多画风防呆：启用画风≤1 禁止关闭；⑥关多画风默认自动置顶+「设为默认」按钮；⑦新建画风自动绑定用途/加急（原遗漏，已修为铁律，SPEC §6）；⑧新建画风按钮放大并入多画风栏；⑨分栏阈值 540→680px。后端：createArtStyle 自动绑定 usage/rush。
> 🔍 **布局审计**（huiyue-layout-audit 技能）：截图 4 宽度落盘 docs/audit-screenshots/price-round1-*.png；VL 两轮评审 C1-C4/S1-S5 均无阻塞；measure 收敛至本文件存量值（锁提示/导入弹窗区未触及，如实记录）。
> 🔴 **v51 迁移（上一轮）**：绑定行脏快照清洗已生效（生产 7 行已清）；快照仅解绑行生效。
> 🔴 **v51 热修根因（用户截图报障）**：v50 迁移把已绑定行的快照列写入错误默认值(fixed/add)，遮蔽模板真实值 → 系统种子「加急」被显示成「加急 ¥100 开关」落进普通增项组；且新建弹窗允许用途/加急选个数控件。修复：迁移 v51 清洗绑定行脏快照（生产 7 行已清，模板为唯一权威）+ 查询改 CASE WHEN 快照仅解绑行生效 + 后端用途/加急强制开关控件硬校验 + 弹窗锁开关 + 尺寸行布局重排（去丑占位块，三行结构）+ 池子胶囊类别色。新增 TC-V51×3 回归。
> ✅ **SPEC-PRICE-2 全链**：批1 `856055a` / 批3 `777a9a5` / 批4 `7be8b27` / 批5 `ae9f7ed` / v51 热修 `d88465e`。门禁：server **1005** · web **254** · **E2E 7/7** · tsc 0 · eslint 0 · check-i18n 0。
> 🔴 **公式铁律（用户拍板，全链路已统一）**：最终价 = (基础价 + Σ固定增项 + Σ百分比增项[只按基础价]) × 用途 × 加急 × 折扣；全程整数分防 IEEE 754；增项两类控件（开关/个数，**用途/加急强制开关**）× 两种计价（¥/%）× 三类 category；用途/加急下单各选一个（后端 ADDON_SELECTION_MUTEX 兜底）。
> ⚠️ **待用户复验**：画师端「价格管理」——「加急」应回到加急组显示 +100%；新建弹窗用途/加急无个数控件；尺寸行布局已重排。
...
- **HEAD**：批6 提交（与 origin 同步）
- **工作树**：主仓干净（无活跃 worktree）
- **测试基线**：server **1005/1005**（68 文件）· web **254/254**（17 文件）· E2E 7/7 · tsc 0 · eslint 0 · check-i18n 0 · web build 通过
- **后端 100% TS + strict 全开 + any 清零**（唯一豁免 init.js @ts-nocheck）
- **版本**：npm 0.45.0（SPEC-PRICE-2 重构未单独发版，收编待用户验收后定 v0.46）
- **容器**：✅ **已重建 = 批6 最新**（含新 UI 与 v51 迁移；备份 `bak-pre-v050-pricemodel-20260809` + 迁移器自动 .bak.v50 在）
- **迁移**：**v51**（v50 快照污染清洗）为最新；**规范**：`docs/specs/SPEC-PRICE-2-价格模型统一重构.md`（公式/模型唯一事实源）
...
**排队中（验收通过后）**：宿主 cron 每日备份配置（OPS.md §2）；巨型组件拆分 Top5（OrderDetail/QueueBoardCalendar 等）；formatYuan 统一；repowiki P2；v0.46 收编发版。
