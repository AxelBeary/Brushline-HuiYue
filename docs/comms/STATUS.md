...
- **迁移**：v42（F2 社交平台表）为最新
- **comms**：STATUS + 五号视觉巡检批派工（`01-to-05-视觉巡检批-20260805.md`，⚠️ 用户称已取消，待确认）+ 五号两份核实报告（安全审计/前端优化，待排期消费）；二号手动录单日期批派工/交付已消费删除（合入即删）
...
**在途情况（2026-08-06 刷新）**：五号视觉巡检批用户称已取消（待最终确认，确认后派工文件作废删除）；二号手动录单日期批已合入（`9815a3c`）。二三四五号均待派。
...
| 角色 | 状态 |
|------|------|
| 二号 | ✅ **手动录单日期批已合入（`9815a3c`）**：B1 今天可选 / B2 截稿↔开稿双向灰掉+提交拦截 / B3 参考图标签补齐（一号独立复跑 web215 全绿）→ 待派 |
| 三号 | ✅ addons 收尾批已合入（`58d48c9`，旧增项体系完全清退）→ 待派 |
| 四号 | ✅ 说明书 v0.39 已合入（`ece9310`）→ 待派 |
| 五号 | ✅ 安全加固批已合入（`ee0f68a`）。⚠️ 视觉巡检批：用户 2026-08-06 口头声明"之前取消了"，STATUS 与 git 内均无取消记录——**待用户确认**：确认取消则删除派工文件 `01-to-05-视觉巡检批-20260805.md`，本项作废 |
