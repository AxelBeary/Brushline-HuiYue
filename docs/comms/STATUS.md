...
> 最后更新：2026-08-08 v36（五号时间条月历三连合入）——**时间条/月历三连已合入**（master `0ecf281`：批F拖拽平移+画布6000px+保中心 / 批F2滚轮+pinch缩放+changeTlZoom统一 / 批G可接单角标+日视图弹窗+跳月，QueueBoardCalendar.vue +312/-41 + locales，门禁全过 + 浏览器冒烟 15/15）。五号空闲，w5 已清理。**并行剩一路执行中**：二号管理后台导航分组（w2 `beta/admin-nav-groups`）。master `0ecf281` 同步。**容器落后 v0.43** 待收编重建。
