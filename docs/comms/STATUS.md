...
> 🔑 **新会话接手指南**：无在途施工。v135〜v139 已合入 master（63363bc6）并进本机生产（部署全绿）；开放项：用户公网跑 update.sh 同步后复验六项（账号与安全页日历订阅/时间条全年档与仅进行中过滤与临期藤黄色/价目卡导入与双布局/统计导出页收入趋势图/小票打印机导 PNG）+ v134 遗留（公网 Passkey 重注册）；吸纳缓行四项收口（**2026-08-20 用户拍板**）：①订单模板库更名「快捷开单」（去掉模板字样），做/不做本身未拍板、增量价值仍待议；②仪表盘拖拽布局**拍板要做**——B 档：组件换位 + 显隐开关合并为「自定义我的首页」（不做卡片缩放：纸墨纸边卡片缩放易破相、仪表盘无图表块收益存疑），**交互补充拍板（820 原型复验）：首页卡片固定不可拖，自定义入口收进设置/偏好页，排序与显隐只在该页操作（原型 proto-dashboard-drag-820.html 已按此改）**，先出可交互原型供用户浏览器拍板再施工；③时薪统计**移入桌面端预案**（REQ-014 Tauri 路线，桌面端做自动计时完整版），网页端不做——工具箱手动记时先搞方案经呈现利弊后未被选中；④着色五维持续挂起（已核实门槛下调：拾绘画风+尺寸价目体系已覆盖半数类别概念，将来做可直接按画风着色不新造字段）；另挂起：首页视觉原型（两轮十一款未相中，用户拍板过两天再说，重开从 I 续编）。
