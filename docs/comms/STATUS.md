...
> 最后更新：2026-08-09 v48（**SPEC-PRICE-2 价格模型统一重构——批1后端+批3画师端前端已合入**）——master 与 origin 同步。**容器未重建**（等批4 客户侧完成，见风险）。
> 🔴 **用户指令（08-09）**：批准推进 SPEC-PRICE-2 全批次（画师端/客户侧/视觉），约束：代码质量铁律、客户界面直观本能、全链路语义一致、不沿用旧产出、每批完成后更新 STATUS + 阶段汇报等确认。
> 🔴 **高风险提示**：批1 已删除旧倍率/档位 API，但客户端 OrderForm/ManualOrder 仍在调用旧 API（批4 重写）——**批4 合入前禁止重建容器**，否则线上客户下单链路报错。
> ⚠️ **已解决（原归档三大拍板项）**：①02H 价格三类合并 → 升级为 SPEC-PRICE-2 真 category 模型（名称约定彻底退役）②倍率 tab 已彻底移除（MultiplierManager 删除）③视觉方向待批5 按用户验收推进。
> ✅ **已合入（08-09）**：批1 `856055a`（引擎全整数分重写 + 迁移v50 + 旧表清退 + 999→1002 测试）/ 批3（画师端价格管理重做：增项库全维度CRUD + 三态落库 + 预览公式重写 + 解绑端点 + 视觉token化）。
> ✅ **全部角色空闲**（无活跃 worktree/分支）。
...
- **HEAD**：批3 提交（推送后同步 origin）
- **工作树**：主仓干净（无活跃 worktree）
- **测试基线**：server **1002/1002**（68 文件）· web **262/262**（17 文件）· tsc 0 · eslint 0 · oxlint 基线 · check-i18n 0 · web build 通过
- **后端 100% TS + strict 全开 + any 清零**（唯一豁免 init.js @ts-nocheck）
- **版本**：npm 0.45.0（SPEC-PRICE-2 重构未发新版本，待批4/5 完成后统一收编 v0.46）
- **容器**：⚠️ **仍为 v0.45 旧代码**——批1 删了旧倍率/档位 API，客户侧 OrderForm（批4）适配前重建会断下单链路
- **备份**：`bak-pre-v050-pricemodel-20260809`（批1 迁移前，integrity ok）+ 更早里程碑；03E backup-db.ts 每日备份能力（宿主 cron 待配置，见 OPS.md）
- **迁移**：**v50**（SPEC-PRICE-2：orders 重建去 tier/旧倍率列 + style_size_id / addon_templates category+price_mode / breakdown 新口径 / DROP price_tiers+price_multipliers）为最新
- **规范**：`docs/specs/SPEC-PRICE-2-价格模型统一重构.md`（公式/模型唯一事实源）
...
## 当前阶段（2026-08-09 用户批准）**SPEC-PRICE-2 价格模型统一重构**——公式铁律：(基础价+Σ固定增项+Σ百分比增项[只基于基础价])×用途×加急×折扣，全程整数分。

**批次进度**：
- ✅ **批1 后端地基**（`856055a`）：引擎全整数分重写（用途/加急=category 增项，后端互斥单选）；迁移 v50；旧档位/旧倍率表+API+服务全清退；订单/队列/健康检查 JOIN 全切画风尺寸；新端点无，测试 1002 绿
- ✅ **批3 画师端前端**：增项库全维度 CRUD（类别/控件/计价/数量上限）；新建弹窗新模型（修复 02H 字段静默剥离断级 bug）；三态 display_status 落库；预览按新公式重写（用途/加急可选展示）；解绑真实落库（新 DELETE 端点）；覆盖预载改 GET 只读端点；倍率 tab + MultiplierManager 删除；圆角 token 化 + 双列 grid；中英 i18n 全键
- ⏳ **批4 客户侧**：OrderForm/ManualOrder 重写（用途单选/加急单选/普通增项多选 + 直观价格明细），适配新 API（**完成前禁止重建容器**）；旧模型测试 fixture 同步重写
- ⏳ **批5 视觉与测试**：客户端报价展示直观化、E2E 钱链路用例、before/after 截图门禁、容器重建（需用户确认）

**待用户拍板**：批3 画师端视觉验收（可本地起 dev 预览）；批5 前客户侧视觉方向细节
