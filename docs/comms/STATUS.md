...
> 最后更新：2026-08-09 v50（**SPEC-PRICE-2 v51 热修已合入并重建容器，待用户复验**）——master `d88465e` 与 origin 同步。**容器 = master 最新**（迁移 v51 已生效）。
...
- **HEAD**：`d88465e`（v51 热修，与 origin 同步）
...
- **容器**：✅ **已重建 = `d88465e`**（迁移 v51 已生效，生产 7 行脏快照已清洗；备份 `bak-pre-v050-pricemodel-20260809` + 迁移器自动 .bak.v50 在）
