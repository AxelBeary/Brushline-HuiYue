...
> 最后更新：2026-08-07 v15——**批4+低垂果实批全部合入，容器已刷新，待用户重启 Hermes 验证视觉 API**。基线 server **930/930** · web **215/215**。HEAD `e3f8a16`。
> ⚠️ **待用户操作**：视觉 API 配置已确认正确（config.yaml auxiliary.vision = 阿里云 MaaS qwen3.8-max，provider/base_url/api_key 全齐）但当前会话仍走旧 chatanywhere（报 404）——**需重启 Hermes 或 /new 让辅助模型配置生效**。若重启后仍失败查 `agent.image_input_mode: text`。重启/新会话前：低垂果实批已全部合入（无在途子代理），可安全刷新。
...
- **HEAD**：`e3f8a16`（低垂果实批合入即删），与 origin 同步
- **工作树**：主仓干净
...
## 当前阶段（2026-08-06 用户定）**Beta 冲刺**——修 Bug + 视觉问题，目标几天内发布 Beta。**批 1-批 4b + 低垂果实批全部合入**，容器已刷新到最新。**待办池见下**。

**冲刺待办池（刷新后据此派工）**：
- **F3 快捷入口实施批（用户已拍板，待落档实施）**：默认 5 个（新建订单/预览主页/约稿须知编辑/分享接稿页(复制链接)/快捷入口设置）；状态切换做循环按钮（点一下 open→full→break，展示当前状态）并删下方 StatusSwitch；快速发作品=画师直接拖图/粘贴到块（真正人类逻辑）；收入概览不做（旁边已有收入大块，用户甚至想缩小/挪标签）；未读留言不做（下方已有留言板块）；保留自定义能力+最好自定义布局；方案依据 REQ-034。
- **Bug/视觉**：
  ...
  - 内测反馈批 4 ✅ F2对比度/F5a观感/F1字号/F4上传区/F6草稿 全合入；**F3 快捷入口**见上
  - 视觉截图待用户口述验收（beta-od01 五色换色 / beta-tags 标签 / beta-templates 模板 / beta-batch4 批4，部分 after 待视觉 skill 就绪后补拍）
  - 色板联动缺口（Settings ACCENT_PRESETS 旧 hex vs 后端白名单旧值）——批 5 三号候选
