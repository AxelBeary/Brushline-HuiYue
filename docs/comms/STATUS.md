...
> 最后更新：2026-08-09 v49（**SPEC-PRICE-2 价格模型统一重构全五批完成，容器已重建待用户验收**）——master 与 origin 同步。**容器 = master 最新**（v50 迁移已随重建生效，备份 `bak-pre-v050-pricemodel-20260809`）。**AUTH_DEV_MODE=false**。
> ✅ **SPEC-PRICE-2 五批全部合入**：批1 `856055a`（后端地基：引擎全整数分+迁移v50+旧表清退）/ 批3 `777a9a5`（画师端：增项库全维度+三态落库+预览重写+解绑）/ 批4 `7be8b27`（客户侧：三区选择+直观明细+管理端抽屉适配）/ 批5 `ae9f7ed`（E2E 钱链路 E6b）。
> 🔴 **公式铁律（用户拍板，全链路已统一）**：最终价 = (基础价 + Σ固定增项 + Σ百分比增项[只按基础价]) × 用途 × 加急 × 折扣；全程整数分防 IEEE 754；增项两类控件（开关/个数）× 两种计价（¥/%）× 三类 category（add/usage/rush）；用途/加急各单选（后端 ADDON_SELECTION_MUTEX 兜底）。
> ⚠️ **待用户验收**：重建后容器即新模型——重点看：画师端「价格管理」页（倍率tab已删，增项库全维度）、客户端下单三区选择+价格明细、展示态尺寸拒单。不满意按批回滚（提交链清晰）。
...
- **HEAD**：批5 提交（推送后同步 origin）
- **工作树**：主仓干净（无活跃 worktree）
- **测试基线**：server **1002/1002**（68 文件）· web **254/254**（17 文件）· **E2E 7/7**（含新钱链路 E6b）· tsc 0 · eslint 0 · check-i18n 0 · web build 通过
- **后端 100% TS + strict 全开 + any 清零**（唯一豁免 init.js @ts-nocheck）
- **版本**：npm 0.45.0（SPEC-PRICE-2 重构未单独发版，收编待用户验收后定 v0.46）
- **容器**：✅ **已重建 = master 最新**（v50 迁移随重建生效；旧 price_tiers/price_multipliers 已 DROP，迁移前备份在）
- **备份**：`bak-pre-v050-pricemodel-20260809`（批1 迁移前，integrity ok）+ 更早里程碑；03E backup-db.ts 每日备份能力（宿主 cron 待配置，见 OPS.md）
- **迁移**：**v50**（SPEC-PRICE-2）为最新；**规范**：`docs/specs/SPEC-PRICE-2-价格模型统一重构.md`（公式/模型唯一事实源）
