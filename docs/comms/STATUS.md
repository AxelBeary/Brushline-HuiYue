...
> 🔜 **在途**：813-fq-tail-anim（收尾波 T：hover 位移/循环/光晕动效纪律清违+残留硬编码 token 化+列表增删过渡，codex 施工中）。波 M（客户端三态散项+a11y+步骤过渡）任务书已备（813-fq-tail-logic-task.md），与波 T 同文件交叉，待 T 合入后派防冲突。
> 🔜 **待办**：波 T 验收合入→派波 M→验收合入→容器重建+三域终局截图交用户终验。
